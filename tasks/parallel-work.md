...
| `/root/design_multi_case_schema` (`case-schema-sept10`, continued by `/root/review_f2_conditional`) | F2 entitlement case-identity expansion | Exact eight classified PostgreSQL compatibility test files plus `phpunit.organization-postgres.xml`; candidate production/focused files remain frozen | Repair rollback dependency order and exact-case fixtures, then require the full disposable PostgreSQL suite to pass | Schema `127e578` + `7cbc587` is integrated as `45f4a94` + `73660fb`; dual-write `f2e7a29` is integrated as `355dab8`. Production candidate `7a80180` + `f906cf9` is independently design-approved: generic entitlements bind the exact INTEGRATED case, DASS remains case-less, the new guard preserves DIRECT_PUBLIC/LEGACY, and unsafe down fails closed. Focused SQLite is 16/127, isolated PostgreSQL is 5/35, PHPStan/Pint/diff are green. Full PostgreSQL reproduced 21/530 compatibility/order errors plus variable unrelated broken-pipe noise, so integration remains blocked while the classified test/runner slice is active; historical migrations, old uniqueness, grants/sessions/readers, shared routes/contracts, and candidate production files remain frozen |
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, continued by `/root/review_f9_lifecycle`) | F9 audit-retention repair | No active ownership after the accepted assessment-activation audit slice | Select the next real audit writer; do not reclassify outbox/business retention | Manual proof through invoice-claim slices are integrated through `f2add06`; integrated-checkout is integrated as `b5adb83`. Checkout lifecycle, handoff issuance, and session establishment are integrated as `079f9bc`, `91b048d`, and `5a15950`; invoice outcome/permit and reservation audits are integrated as `e888155`, `ab3411f`, and `1ce2cce`. Assessment activation audit worker `1c0a8b5` passed independent review and is integrated as `5b90b0f`; main focused test passed 30/113 and reviewer downstream gate passed 59/1,918. All accepted slices have green Pint/diff and no new PHPStan findings. Outbox two-year retention remains unchanged; `ProvisionAssessmentParticipant.php` remains exclusively in the blocked F2 candidate and no migration, route, config, provider call, or live integration is owned here |
