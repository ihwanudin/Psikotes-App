...
- Admin: Supabase Auth email+password; role di `admins` (super_admin | branch_admin); reviewer/HR = branch_admin read-only (flag `can_verify=false`).
...
## Data
- Enkripsi: TLS in-transit; at-rest = enkripsi bawaan Supabase/R2. **Enkripsi kolom aplikasi untuk skor TIDAK diterapkan** — merusak query dashboard/rekap dan kuncinya toh harus dipegang Worker yang sama; kontrol yang benar di sini = RLS ketat + signed URL + audit akses. (Koreksi atas poin A5 list.)
