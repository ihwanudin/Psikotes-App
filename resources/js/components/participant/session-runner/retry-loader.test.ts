import assert from 'node:assert/strict';
import { test } from 'node:test';

import { createRetryLoader } from './retry-loader.ts';

// A minimal synthetic outcome shape for testing the core in isolation,
// with the one property every real per-instrument outcome union shares:
// a 'network_error' variant recognized by isNetworkError below.
type FakeOutcome =
    | { type: 'available'; value: string }
    | { type: 'not_found' }
    | { type: 'network_error' };

function isNetworkError(outcome: FakeOutcome): boolean {
    return outcome.type === 'network_error';
}

function fakeQueueRetry(): {
    queueRetry: (retry: () => void) => () => void;
    fireQueuedRetry: () => void;
    queuedCount: number;
} {
    let queued: (() => void) | null = null;
    let queuedCount = 0;

    return {
        queueRetry(retry) {
            queued = retry;
            queuedCount++;

            return () => {
                if (queued === retry) {
                    queued = null;
                }
            };
        },
        fireQueuedRetry() {
            queued?.();
        },
        get queuedCount() {
            return queuedCount;
        },
    };
}

test('starts in loading and resolves to ready on a final outcome', async () => {
    const outcome: FakeOutcome = { type: 'not_found' };
    const { queueRetry } = fakeQueueRetry();
    const loader = createRetryLoader<FakeOutcome>({
        fetch: async () => outcome,
        queueRetry,
        isNetworkError,
    });

    assert.deepEqual(loader.getState(), { status: 'loading' });
    loader.start();
    await Promise.resolve();

    assert.deepEqual(loader.getState(), { status: 'ready', outcome });
});

test('an isNetworkError outcome transitions to reconnecting and queues a retry, not a terminal failure', async () => {
    const tracker = fakeQueueRetry();
    let calls = 0;
    const loader = createRetryLoader<FakeOutcome>({
        fetch: async () => {
            calls++;

            return { type: 'network_error' };
        },
        queueRetry: tracker.queueRetry,
        isNetworkError,
    });

    loader.start();
    await Promise.resolve();

    assert.deepEqual(loader.getState(), {
        status: 'reconnecting',
        autoRetryExhausted: false,
    });
    assert.equal(calls, 1);
    assert.equal(
        tracker.queuedCount,
        1,
        'a network_error outcome must queue exactly one retry',
    );
});

test('a rejected/thrown fetcher (a real dropped-signal failure) is treated identically to an isNetworkError outcome', async () => {
    const tracker = fakeQueueRetry();
    const outcomes: (() => Promise<FakeOutcome>)[] = [
        () => Promise.reject(new TypeError('Failed to fetch')),
        () => Promise.resolve({ type: 'available', value: 'x' }),
    ];
    let call = 0;
    const loader = createRetryLoader<FakeOutcome>({
        fetch: () => outcomes[call++]!(),
        queueRetry: tracker.queueRetry,
        isNetworkError,
    });

    loader.start();
    await Promise.resolve();
    await Promise.resolve();

    assert.deepEqual(
        loader.getState(),
        { status: 'reconnecting', autoRetryExhausted: false },
        'a rejected fetch must be treated as a connectivity failure, not a terminal error',
    );

    tracker.fireQueuedRetry();
    await Promise.resolve();

    assert.deepEqual(loader.getState(), {
        status: 'ready',
        outcome: { type: 'available', value: 'x' },
    });
});

test('network_error -> connectivity restored -> retry fires -> available yields the ready state with the correct outcome', async () => {
    const tracker = fakeQueueRetry();
    const outcomes: FakeOutcome[] = [
        { type: 'network_error' },
        { type: 'available', value: 'x' },
    ];
    let call = 0;
    const loader = createRetryLoader<FakeOutcome>({
        fetch: async () => outcomes[call++]!,
        queueRetry: tracker.queueRetry,
        isNetworkError,
    });

    loader.start();
    await Promise.resolve();
    assert.deepEqual(loader.getState(), {
        status: 'reconnecting',
        autoRetryExhausted: false,
    });

    tracker.fireQueuedRetry();
    await Promise.resolve();

    assert.deepEqual(loader.getState(), {
        status: 'ready',
        outcome: outcomes[1],
    });
});

test('a final outcome is never retried', async () => {
    const tracker = fakeQueueRetry();
    const outcome: FakeOutcome = { type: 'not_found' };
    const loader = createRetryLoader<FakeOutcome>({
        fetch: async () => outcome,
        queueRetry: tracker.queueRetry,
        isNetworkError,
    });

    loader.start();
    await Promise.resolve();

    assert.deepEqual(loader.getState(), { status: 'ready', outcome });
    assert.equal(
        tracker.queuedCount,
        0,
        'a final outcome must never queue a retry',
    );
});

test('automatic retries are capped, and the counter is shared across isNetworkError outcomes and rejections', async () => {
    const tracker = fakeQueueRetry();
    let calls = 0;
    const loader = createRetryLoader<FakeOutcome>({
        fetch: () => {
            calls++;

            return calls % 2 === 0
                ? Promise.reject(new TypeError('Failed to fetch'))
                : Promise.resolve({ type: 'network_error' as const });
        },
        queueRetry: tracker.queueRetry,
        isNetworkError,
    });

    loader.start();
    await Promise.resolve();
    await Promise.resolve();

    for (let i = 0; i < 10; i++) {
        const state = loader.getState();

        if (state.status === 'reconnecting' && state.autoRetryExhausted) {
            break;
        }

        tracker.fireQueuedRetry();
        await Promise.resolve();
        await Promise.resolve();
    }

    assert.deepEqual(loader.getState(), {
        status: 'reconnecting',
        autoRetryExhausted: true,
    });
    assert.equal(
        calls,
        5,
        'exactly 5 consecutive failures (the cap) should have been attempted',
    );
    assert.equal(
        tracker.queuedCount,
        4,
        'the 5th failure must not queue a 6th automatic retry',
    );

    tracker.fireQueuedRetry();
    await Promise.resolve();
    assert.equal(calls, 5);
});

test('after the automatic-retry cap is reached, a manual retry() still works and resets the budget', async () => {
    const tracker = fakeQueueRetry();
    let calls = 0;
    const loader = createRetryLoader<FakeOutcome>({
        fetch: () => {
            calls++;

            if (calls <= 5) {
                return Promise.resolve({ type: 'network_error' as const });
            }

            return Promise.resolve({ type: 'not_found' as const });
        },
        queueRetry: tracker.queueRetry,
        isNetworkError,
    });

    loader.start();

    for (let i = 0; i < 5; i++) {
        await Promise.resolve();
        await Promise.resolve();

        if (i < 4) {
            tracker.fireQueuedRetry();
        }
    }

    assert.deepEqual(loader.getState(), {
        status: 'reconnecting',
        autoRetryExhausted: true,
    });
    assert.equal(calls, 5);

    loader.retry();
    assert.deepEqual(loader.getState(), { status: 'loading' });
    await Promise.resolve();

    assert.deepEqual(loader.getState(), {
        status: 'ready',
        outcome: { type: 'not_found' },
    });
    assert.equal(
        calls,
        6,
        'retry() must have triggered exactly one more attempt',
    );
});

test('retry() bypasses the queued wait and re-attempts immediately', async () => {
    const tracker = fakeQueueRetry();
    const outcomes: FakeOutcome[] = [
        { type: 'network_error' },
        { type: 'not_found' },
    ];
    let call = 0;
    const loader = createRetryLoader<FakeOutcome>({
        fetch: async () => outcomes[call++]!,
        queueRetry: tracker.queueRetry,
        isNetworkError,
    });

    loader.start();
    await Promise.resolve();
    assert.deepEqual(loader.getState(), {
        status: 'reconnecting',
        autoRetryExhausted: false,
    });

    loader.retry();
    assert.deepEqual(
        loader.getState(),
        { status: 'loading' },
        'retry() must show loading immediately, not wait for the fetch',
    );
    await Promise.resolve();

    assert.deepEqual(loader.getState(), {
        status: 'ready',
        outcome: outcomes[1],
    });
});

test('a stale attempt superseded by retry() never overwrites the newer result', async () => {
    const tracker = fakeQueueRetry();
    const first = Promise.withResolvers<FakeOutcome>();
    let call = 0;
    const loader = createRetryLoader<FakeOutcome>({
        fetch: () => {
            call++;

            return call === 1
                ? first.promise
                : Promise.resolve({ type: 'not_found' as const });
        },
        queueRetry: tracker.queueRetry,
        isNetworkError,
    });

    loader.start();
    loader.retry();
    await Promise.resolve();
    await Promise.resolve();

    assert.deepEqual(loader.getState(), {
        status: 'ready',
        outcome: { type: 'not_found' },
    });

    first.resolve({ type: 'available', value: 'stale' });
    await Promise.resolve();
    await Promise.resolve();

    assert.deepEqual(
        loader.getState(),
        { status: 'ready', outcome: { type: 'not_found' } },
        'the first (stale) attempt resolving late must not overwrite the newer ready state',
    );
});

test('dispose() cancels a queued retry and ignores a still-in-flight resolution', async () => {
    const tracker = fakeQueueRetry();
    const deferred = Promise.withResolvers<FakeOutcome>();
    const loader = createRetryLoader<FakeOutcome>({
        fetch: () => deferred.promise,
        queueRetry: tracker.queueRetry,
        isNetworkError,
    });

    loader.start();
    await Promise.resolve();
    loader.dispose();

    deferred.resolve({ type: 'not_found' });
    await Promise.resolve();
    await Promise.resolve();

    assert.deepEqual(
        loader.getState(),
        { status: 'loading' },
        'a resolution arriving after dispose() must never update state',
    );
});
