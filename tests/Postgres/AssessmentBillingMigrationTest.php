...
{
    public function test_result_ledger_fixture_preserves_reverse_down_and_forward_up_order(): void
    {
        ...
        try {
            $before = $this->genericLedgerState();
            $this->assertTrue(Schema::hasTable('generic_instrument_results'));
            GenericResultLedgerMigrationFixture::withoutLedger(function (): void {
                $this->assertFalse(Schema::hasTable('generic_instrument_results'));
                (require database_path('migrations/2026_09_09_000700_create_test_session_grants.php'))->down();
                $this->assertFalse(Schema::hasTable('test_session_grants'));
            ...
            try {
                GenericResultLedgerMigrationFixture::withoutLedger(
                ...
                $this->fail('The synthetic callback failure was not propagated.');
            } catch (\RuntimeException $exception) {
                $this->assertSame('synthetic callback failure', $exception->getMessage());
            }
            $this->assertTrue(Schema::hasTable('generic_instrument_results'));
            $this->assertSame($before, $this->genericLedgerState());

            $ledger = require database_path(
                'migrations/2026_09_13_000100_create_generic_instrument_result_ledger.php',
            );
            $ledger->down();
            try {
                $this->assertSame('absent', GenericResultLedgerMigrationFixture::withoutLedger(
                    static fn (): string => 'absent',
                ));
                $this->assertFalse(Schema::hasTable('generic_instrument_results'));
            } finally {
                $ledger->up();
    ...
    private function genericLedgerState(): array
    {
        return [
            'tables' => array_map(static fn (object $row): array => (array) $row, DB::select(<<<'SQL'
                SELECT class.relname, class.relrowsecurity, class.relforcerowsecurity
                FROM pg_class class
                WHERE class.relname IN ('generic_instrument_results', 'generic_instrument_result_sources')
                ORDER BY class.relname
                SQL)),
            'constraints' => array_map(static fn (object $row): array => (array) $row, DB::select(<<<'SQL'
                SELECT conname, pg_get_constraintdef(oid) AS definition
                FROM pg_constraint
                WHERE conrelid IN ('generic_instrument_results'::regclass, 'generic_instrument_result_sources'::regclass)
                ORDER BY conname
                SQL)),
            'indexes' => array_map(static fn (object $row): array => (array) $row, DB::select(<<<'SQL'
                SELECT indexname, indexdef FROM pg_indexes
                WHERE schemaname = 'public'
                  AND tablename IN ('generic_instrument_results', 'generic_instrument_result_sources', 'instrument_versions')
                  AND indexname LIKE '%instrument_result%'
                ORDER BY indexname
                SQL)),
            'parent_rows' => DB::table('generic_instrument_results')->count(),
            'child_rows' => DB::table('generic_instrument_result_sources')->count(),
        ];
    }
