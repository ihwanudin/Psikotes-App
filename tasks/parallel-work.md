...
| F2 instrument session-definition authority audit | `/root/audit_direct_case_identity` | Read-only SPEC/PRD/config/data inspection | Locate authoritative duration/config/seed/version inputs required by the allocator | active; no code or migration ownership |
