...
| F2 test-session grant schema `000700` | `/root/verify_pg_history` | Sole serial migration owner plus new SQLite/PostgreSQL schema-security tests and explicit runner registration | Add immutable typed grant binding for future allocator dual-write without guessing historical sessions | repair-required after `dea05e2`: direct/legacy graph parity, lock order, exact catalog validation, isolated DASS proof, and integrated/legacy coverage; PostgreSQL also remains unknown |
