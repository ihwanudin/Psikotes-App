...
| `/root/wave1_s4_pg` (reused) | F2 R2 immutable IST result ledger; sole migration owner | `codex/f2-r2-ist-result-ledger`; `C:/Users/ThinkPad/.codex/worktrees/r2-wave1/Psikotes` | integration contains `c2a3641` + `0a85d3a`; repair baseline `c6ec235` | migration `2026_09_13_000100_create_generic_instrument_result_ledger.php` and its PostgreSQL test only while R2a is reopened; R2b writer files are quarantined and frozen | Repair the exact-state verifier and add real two-connection concurrency proof before any writer work resumes. No reuse of Selection result history; no DASS, A1-D5, correction/retest, route, queue, report, or activation. | **R2a REPAIR-REQUIRED; R2b FROZEN.** QA reproduced PG 12/97 but found boolean ACL checks that admit grant options, missing `condeferrable`/`condeferred` verification, and serial-only duplicate evidence. Adversarial RED, minimal schema/test repair, fresh PG, exact cleanup, and QA re-verdict are required |
