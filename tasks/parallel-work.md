...
|---|---|---|---|---|
| `01a0881b-8590-7123-a9ce-640f7cbf75c3` (`2469`) | F2 PostgreSQL and definition catalog | Sole migration owner plus catalog provider/schema/security tests; no allocator/controller/route ownership | Implement the smallest empty-by-default, immutable, versioned SessionDefinition catalog/provider without seeding or activating instrument data | Snapshot EXPAND and its backward-compatibility repair are accepted. CONTRACT readiness audit found no production session/grant writer and correctly blocks CONTRACT until authoritative definitions and dual-write exist. Catalog implementation dispatched; CONTRACT, seeds, allocator wiring, and active DB migration remain excluded |
| `01a0881b-86ec-7fc1-b0f3-99e1e5b99cb1` (`fa77`) | F2 multi-case/retest readiness | Read-only schema/API/domain audit; no migration or implementation ownership while `2469` owns catalog migration | Re-emit the completed multi-case/retest audit because the prior final result was not retrievable by the coordinator | Allocator worker `f6a6f48` + `58a0a81` + `0a24f96` accepted as coordinator `f475a56` + `e07f2ad` + `55d0a59`; independent 152/471, PHPStan 0, Pint/diff clean. Summary-only continuation dispatched; no file edits allowed and route wiring still waits for definition authority |
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, replacement for inactive `01a0881b-85ce-7f43-91d5-dd84fe064064`) | F9 audit-retention repair | `VerifyManualTransfer.php` plus its focused feature test only | Repair the highest-risk mutable audit anchor and replace two-year retention with the five-year Audit policy | Repository inventory completed cleanly at `381b147`: 34 insertions, 4 compliant, 30 two-year violations, no missing expiry, and no raw PII/secrets/URLs/object keys. First bounded repair dispatched with leap-day, immutable-anchor, context, and replay gates |
