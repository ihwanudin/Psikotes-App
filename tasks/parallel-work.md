...
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, continued by `/root/review_f9_lifecycle`) | F9 audit-retention repair | `IssueCheckoutHandoff.php` plus its focused test only | Replace only its hard-coded two-year audit expiry with the five-year Audit policy without changing handoff TTLs | Manual proof through invoice-claim slices are integrated through `f2add06`; integrated-checkout is integrated as `b5adb83`. Checkout-session lifecycle worker `2910ce5` was independently accepted (23/261, PHPStan/Pint/diff green) and integrated as `079f9bc`. The next handoff-audit slice is active from worker baseline `2910ce5`; no migration, route, config, provider call, or live integration ownership |
