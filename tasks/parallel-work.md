...
| F2 origin-aware case authorization resolver | `/root/audit_direct_case_identity` | New read-only resolver/domain files and focused tests only | Resolve INTEGRATED, DIRECT_PUBLIC, and LEGACY_SELECTION grants to one exact case; fail closed without wiring a session writer | active from baseline `3b11c87`; no migration or shared-contract ownership |
| F2 resolver contract adversarial audit | `/root/audit_legacy_case_identity` | Read-only schema/model/query inspection | Freeze cardinality, tenant, instrument, origin, and DASS-only rejection semantics before integration review | active from baseline `3b11c87`; no file ownership |
| F9 secret/PII scan gate readiness audit | `/root/verify_pg_history` | Read-only CI/script/test inventory | Define a bounded synthetic scanning gate and safe exclusive files for a later increment | active from baseline `3b11c87`; independent of F2 and no file ownership |
