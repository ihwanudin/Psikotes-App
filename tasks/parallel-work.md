...
| `/root/design_multi_case_schema` (`case-schema-sept10`) | F2 entitlement case-identity expansion | One new conditional-invariant migration and new dedicated SQLite/PostgreSQL tests only | Require case identity for every generic entitlement while DASS remains case-less | Schema `127e578` + `7cbc587` is integrated as `45f4a94` + `73660fb`; dual-write worker `f2e7a29` was independently accepted and integrated as `355dab8` (main 24/194). All main PHPStan/Pint/diff gates are green; worker PostgreSQL passed 9/112 for dual-write. Conditional-invariant slice 3/6 is active from worker baseline `f2e7a29`; it must not contract old uniqueness, modify grants/sessions/readers/writers, or wire runtime |
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, implementation resumed by `/root/rereview_f9_manual_proof`) | F9 audit-retention repair | `ConfirmIntegratedCheckout.php` plus its focused consent test only | Replace both hard-coded two-year audit expiries with the five-year Audit policy without changing checkout semantics | Manual proof through bill-finalization slices are integrated through `d590a16`; invoice-claim worker `01b9be9` was independently accepted and integrated as `f2add06` (main 60/452), with its notification TTL intentionally kept at two years. Every slice has PHPStan 0 and green Pint/diff gates. Integrated-checkout audit retention is active from worker baseline `01b9be9`; no migration, route, provider call, or live integration ownership |
