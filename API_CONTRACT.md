...
- `GET  /sessions/:id/items` → `{session_id,instrument,version,subtests:[{code,items:[...]}]}`. Gerbang baca sama seperti `/sessions/:id/answers`: hanya `in_progress` dan sebelum `ends_at` (`SESSION_NOT_STARTED`/`SESSION_CLOSED`/`DEADLINE_EXCEEDED`); tidak pernah menulis; `503 ASSESSMENT_ITEM_CONTENT_UNAVAILABLE` bila belum ada pembaca terdaftar untuk instrumen itu, atau bila pembaca yang terdaftar belum bisa menghasilkan konten (termasuk seluruh IST hari ini — lihat di bawah). Bentuk `items` per instrumen ditentukan pembacanya sendiri, bukan endpoint ini. **Kraepelin** (2026-09-21): 50 subtest `col_01..col_50`, masing-masing berisi tepat 28 `{position,value}` **sudah dalam urutan administrasi (bawah ke atas)** — `position=1` adalah angka paling bawah di lembar cetak, `position=28` paling atas. Sumber `kraepelin_grid.json` tersimpan dalam urutan lembar (baris 0 = atas); server membalik urutannya sekali di `KraepelinItemContentReader` sebelum respons dikirim. Klien TIDAK boleh membalik urutan apa pun lagi — satu pembalikan yang salah tempat memasangkan jawaban peserta dengan pasangan angka yang salah. **IST** (`IstItemContentReader`, F2 IST reader Stage 1, 2026-09-21): subtest `SE/WA/AN` (multiple-choice) → `{code,answer_type:'multiple_choice',instructions,items:[{item,text?,options:{a..e}}]}` (`text` hanya ada bila soal punya kalimat pembuka — `WA` tidak); `GE` → `answer_type:'fill_in_word'`, `items:[{item,text}]`; `RA/ZR` → `answer_type:'fill_in_numeric'`, `items:[{item,text}]`. Kunci jawaban/`match_strategy` TIDAK PERNAH ada di respons ini — hanya di `instrument_versions` code=`ist` (norma skoring), tidak pernah dibaca pembaca item. Seluruh instrumen IST tetap `503 ASSESSMENT_ITEM_CONTENT_UNAVAILABLE` selama `ist_items.json`-nya berstatus `draft` di tingkat atas (hari ini demikian: subtest `ME` masih menunggu keputusan psikolog atas ambiguitas daftar hafalan, dan `FA`/`WU` belum ada sama sekali di `main` — lihat PR #73) — ini juga berarti sesi IST tidak bisa `start` sama sekali selama itu (diperiksa sebelum attempt/grant dialokasikan, lihat bagian start generik di bawah). `ME`, `FA`, `WU` butuh pembaruan `IstItemContentReader` terpisah setelah masing-masing final; `ME` juga butuh mekanisme timer dua-fase yang belum ada (lihat `tasks/handoffs/f2/ist-reader-and-asset-endpoint.md`).
...
- `POST /sessions/:id/subtest/next` (IST) — belum diimplementasikan; desainnya menunggu keputusan psikolog soal apakah waktu membaca petunjuk subtes termasuk jam subtes (lihat `tasks/handoffs/f2/ist-reader-and-asset-endpoint.md`, Titik 4).
