...
| G8 instrument-version source hardening | `/root/review_p17c` | One new migration, InstrumentSeeder trusted service-context wrapper, and isolated Feature/PostgreSQL tests only | FORCE RLS service-only, immutable history, one active version per code, and disposable non-owner verification | queued after coordinator role/seeder freeze; single migration owner |
