...
| `/root/design_multi_case_schema` (`case-schema-sept10`, repair resumed by `/root/review_f2_conditional`) | F2 entitlement case-identity expansion | New conditional-invariant migration/tests plus the exact INTEGRATED provisioning writer and its focused regression test | Repair the writer so generic entitlements receive the just-created case while DASS remains case-less, then re-review the invariant | Schema `127e578` + `7cbc587` is integrated as `45f4a94` + `73660fb`; dual-write worker `f2e7a29` was independently accepted and integrated as `355dab8` (main 24/194). Candidate `7a80180` passed its dedicated SQLite 4/24 and static gates but is rejected standalone: `GenericAssessmentProvisioningTest` failed 7/9 because the active INTEGRATED writer inserted generic entitlements before creating the case. Repair is active on top of `7a80180`; old uniqueness, grants/sessions/readers, shared routes/contracts, and historical migrations remain frozen |
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, continued by `/root/review_f9_lifecycle`) | F9 audit-retention repair | `ReserveAssessmentBill.php` plus its focused test only | Replace only the reservation audit's hard-coded two-year expiry with the five-year Audit policy without changing bill/payment behavior | Manual proof through invoice-claim slices are integrated through `f2add06`; integrated-checkout is integrated as `b5adb83`. Checkout lifecycle, handoff issuance, and session establishment are integrated as `079f9bc`, `91b048d`, and `5a15950` with main focused tests 23/261, 14/171, and 7/122. Invoice outcome and invoice permit audits are integrated as `e888155` (17/283) and `ab3411f` (22/277) after independent review; PHPStan/Pint/diff are green and seven historical test-only PHPStan findings remain baseline-identical. `RecordAssessmentOutcome` remains unchanged because its two-year value is outbox/business retention. Reservation-audit slice is active from worker baseline `957661e`; `ProvisionAssessmentParticipant.php` remains exclusively owned by F2 and no migration, route, config, provider call, or live integration is owned here |
