...
| T-I8 | Bug fungsional ditemukan lewat T-I7 (bukan a11y, tapi terkait langsung): `<img>` `onError` yang gagal terus-menerus (URL "tersedia" tapi objeknya rusak/hilang, atau jaringan HP terus memutus unduhan) sebelumnya tidak pernah berhenti mencoba ulang DAN tidak pernah memunculkan tombol "Coba lagi" — butir jadi tidak bisa dijawab tanpa jalan keluar. ✅ **Diperbaiki**, commit `67c0b00`: `reportImageLoadFailure()` baru di `ist-asset-url-loader.ts`, dibatasi 2 kegagalan berturut-turut sebelum menyerah ke state `error` yang sama; klik manual "Coba lagi" mereset hitungannya. 3 unit test baru + dibuktikan di fixture dengan URL yang selalu gagal dimuat: tepat 2 request jaringan (dihitung di `read_network_requests`, bukan cuma dibaca dari kode), tidak berputar, tombol akhirnya muncul di 44px |
