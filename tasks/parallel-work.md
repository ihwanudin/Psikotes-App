...
| `/root/design_multi_case_schema` (`case-schema-sept10`) | F2 entitlement case-identity expansion | New `2026_09_10_000300` migration and its dedicated SQLite/PostgreSQL tests only | Add nullable generic entitlement case identity, exact DIRECT/LEGACY backfill, DASS exclusion, composite FK, partial uniqueness, immutable guard, and safe rollback refusal | Worker `127e578` was rejected for three Required findings. Corrective child `7cbc587` reports SQLite 9/56, disposable PostgreSQL 15/74, PHPStan 0, and green Pint/diff gates; it adds safe SQLite FK rebuild, exact two-way package composition, and counterfeit catalog/RLS/ACL checks. Independent re-review is active; nothing is integrated. This remains slice 1/6 and must not contract old participant uniqueness or wire runtime |
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, implementation resumed by `/root/rereview_f9_manual_proof`) | F9 audit-retention repair | `SettleZeroPriceCheckout.php` plus its focused SQLite/PostgreSQL tests only | Replace its hard-coded two-year audit expiry with the five-year Audit policy without changing settlement semantics | Manual proof through result-dispatch slices are integrated through `c11e94d`; result-outbox worker `e77f6ca` was independently accepted and integrated as `37a4f57` (main 6/51). Every slice has PHPStan 0 and green Pint/diff gates. Zero-price settlement retention is active from worker baseline `e77f6ca`; no migration, route, provider call, or live integration ownership |
