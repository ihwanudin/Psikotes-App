...
|---|---|---|---|---|
| `01a0881b-8590-7123-a9ce-640f7cbf75c3` (`2469`) | F2 PostgreSQL and snapshot migration | Sole migration owner plus disposable PostgreSQL verification | Hold the later CONTRACT enforcement until allocator writers have switched; audit its exact preconditions without code changes | `000700` accepted as `b7e5c1b`; snapshot EXPAND accepted as `427b4ce` + `d268db1` (workers `a0595a8` + `2593662`), PostgreSQL 24/126 and independent SQLite 107/433 green; CONTRACT deliberately deferred |
| `01a0881b-86ec-7fc1-b0f3-99e1e5b99cb1` (`fa77`) | F2 allocator readiness | Pure allocator contracts; no migration/action/route until dependencies are frozen | Add the definition-authority interface and allocation-result DTO without a production binding | `SessionDefinition::toArray()` accepted as `f3a89f3`; typed `SessionGrantIdentity` accepted as `7144c6b` (worker `04c5b10`), independently verified 11/14 focused and 122/311 relevant Unit, PHPStan/Pint green; next contract slice active |
