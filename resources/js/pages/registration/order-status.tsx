...
> = {
    pending: {
        ...
        description:
            'Pembayaran belum dikonfirmasi. Status akan berubah setelah verifikasi selesai.',
        className: 'border-amber-200 bg-amber-50 text-amber-950',
    },
    paid: {
        label: 'Pembayaran diterima',
        description:
            'Akses tes telah diaktifkan. Gunakan nomor tes yang dikirimkan kepada Anda.',
        className: 'border-teal-200 bg-teal-50 text-teal-950',
    },
    rejected: {
        label: 'Pembayaran ditolak',
        description:
            'Bukti pembayaran perlu diperbaiki sebelum dapat diverifikasi kembali.',
        className: 'border-red-200 bg-red-50 text-red-950',
    },
    expired: {
        label: 'Pembayaran kedaluwarsa',
        description:
            'Batas waktu pembayaran telah berakhir. Hubungi petugas LSI untuk bantuan.',
        className: 'border-slate-200 bg-slate-100 text-slate-900',
    },
    cancelled: {
        label: 'Pesanan dibatalkan',
        description:
            'Pesanan ini tidak lagi aktif. Hubungi petugas LSI jika Anda memerlukan bantuan.',
        className: 'border-slate-200 bg-slate-100 text-slate-900',
    },
};
                ...
                <div className="mx-auto w-full max-w-2xl">
                    <header className="rounded-2xl bg-teal-950 p-7 text-white sm:p-9">
                        ...
                        <p className="mt-3 leading-7 text-teal-50/80">
                            Informasi ini hanya berasal dari sesi pendaftaran
                            Anda.
                        </p>
                    </header>
                    ...
                    <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                        {!authorized ? (
                            <div role="alert" className="text-center">
                                ...
                                <p className="mt-2 text-sm leading-6 text-slate-600">
                                    Buka halaman ini setelah pendaftaran atau
                                    hubungi petugas LSI jika sesi berakhir.
                                </p>
                            </div>
                        ) : order && status ? (
                            <>
                                <div
                                    role="status"
                                    className={`rounded-xl border p-5 ${status.className}`}
                                >
                                ...
                                {order.status === 'rejected' &&
                                    order.rejectionReason && (
                                        <div
                                            role="alert"
                                            className="mt-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm leading-6 text-red-950"
                                        >
                        ...
                        <div className="mt-8 grid gap-3 sm:grid-cols-2">
                            <Button asChild variant="outline">
                                <Link href="/registration/received">
