...
| F2 LEGACY_SELECTION case identity implementation | `/root/audit_legacy_case_identity` | One serial migration, Selection provisioner/model relations, focused SQLite/PostgreSQL/Feature tests | Backfill and enforce exact Selection→case identity, then dual-write new provisioning | active; sole migration owner, no session/direct/result-ledger files |
...
| F2 typed session-definition contract | `/root/audit_direct_case_identity` | New pure domain value objects and synthetic unit tests only | Establish a strict fail-closed definition boundary without inventing production data or wiring allocator | active; no catalog, seeder, migration, controller, or allocator ownership |
