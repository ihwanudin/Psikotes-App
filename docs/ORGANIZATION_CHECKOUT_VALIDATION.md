...
## Checkpoint ACL source-binding dependency pinning — 2026-09-06

Commit `538c144` mem-pin direct sibling dan stdlib dependency callables dengan
module, callable identity/type, serta Python metadata yang diperiksa pre/post
setiap panggilan dan sekali lagi sebelum menerima hasil. Bukti root: **32/32
tests**, `py_compile`, dan independent review **PASS** tanpa P1/P2.

CPython builtins, internal stdlib yang lebih dalam, dan dependency mutation
yang dipulihkan sepenuhnya di dalam callback masih menjadi trusted/residual
...
## Checkpoint pure authority manifest dan broker transport — 2026-09-06
...
ditolak, dan direct SAM membership dipisahkan dari bound runtime token-group
policy. Commit `459e988` menerima pure canonical broker-transport codec
(**11/11 tests**) untuk request, refusal, discard, dan load-sentinel saja.
...
sampai canonical ADR-018 evidence validator/provider diterima. Ini bukan bukti
...
P15/P16/P17c/P18, checklist/progress, gates/payment tidak berubah, tetap terbuka/
default OFF.

## Checkpoint broker-start identity dan structural evidence — 2026-09-06

Commit `b66ebba` menerima pure canonical broker-start identity codec (**8/8
tests**) untuk supplied data saja; live process/token composition binding tetap
wajib. Commit `bb369c6` menerima design boundary ADR-020, lalu `9785eb5`
menerima pure bytes-only structural evidence codec (**12/12 tests**) dengan
hasil explicit `structuralOnly`, bukan native policy/provenance/admission proof.

Suite codec lokal gabungan **56/56** lulus bersama `py_compile`, `diff-check`,
...
Koreksi bukti: run awal pada commit `22a15a1` tidak dipakai sebagai acceptance
setelah review independen menemukan P1/P2 pada custody capability fixture dan
scope postcondition. Commit `764e95b` menutup keduanya: capability kontrol tidak
lagi diteruskan ke command, manifest, report, atau artefak browser; kontrol
berjalan melalui cookie HttpOnly/SameSite strict yang sempit; seluruh output
ditolak bila marker privat muncul; dan sembilan postcondition dibatasi ke
organization fixture yang sedang diuji. Review independen atas koreksi ini
berakhir **PASS** tanpa P1/P2 tersisa.
...
settled. Pada masing-masing run fresh, verifier post-run lulus **9/9**:
profil valid, tepat satu bill organization, paid, 10 allocation settled,
proyeksi READY sesuai profil, entitlement exact, tepat satu invoice intent,
tepat satu audit paid, dan nol write legacy. Browser juga memeriksa alasan empat
row disabled, stale preview,
...
request failure bernilai nol. Pemeriksaan command, report, dan artefak mencatat
**zero capability/private-marker leak**; cleanup selesai dan port loopback yang
dipakai kembali bebas setelah setiap run.
...
Checkpoint terkoreksi ini menutup gap browser cabang `10 -> satu invoice fake ->
seluruh alokasi paid`, tetapi bukan acceptance P17c penuh. Origin masih HTTP
loopback branch-only dan UI peserta pada secure candidate ADR-021/022 belum
