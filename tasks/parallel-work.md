...
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, implementation resumed by `/root/rereview_f9_manual_proof`) | F9 audit-retention repair | `GenericAssessmentResultOutbox.php` plus its focused outbox test only | Replace both hard-coded two-year audit expiries with the five-year Audit policy without changing outbox semantics | Manual proof through poll-projection slices are integrated through `62cabcb`; result-dispatch worker `6865f2d` was independently accepted and integrated as `c11e94d` (main 9/66). Every slice has PHPStan 0 and green Pint/diff gates. Result-outbox retention is active from worker baseline `6865f2d`; no migration, route, provider call, or live integration ownership |
