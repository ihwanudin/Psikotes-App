...
|---|---|---|---|---|
| `01a0881b-8590-7123-a9ce-640f7cbf75c3` (`2469`) | F2 PostgreSQL and snapshot migration | Sole migration owner plus PostgreSQL concurrency design; no allocator implementation | Hold test implementation until core allocator, production definition authority, trusted RLS invocation boundary, and observable phase seams are accepted | `000700` and snapshot EXPAND accepted; CONTRACT still deferred; read-only concurrency design covers 7/7 scenarios and freezes two future test files, but 0/7 are written. Critical blocker: participant request context cannot elevate through `runAsService`, while the resolver requires a service transaction; no artificial bridge is allowed |
| `01a0881b-86ec-7fc1-b0f3-99e1e5b99cb1` (`fa77`) | F2 allocator core | New allocator action and focused SQLite tests only; no migration/route/controller/production authority | Build the unwired first-attempt allocator with fake definition authority, atomic session+grant dual-write, exact replay, and rollback tests | value contracts `f3a89f3`, `7144c6b`, and allocation contracts `527d747` accepted; latest independent gate 11/32 focused and 132/341 AssessmentSessions with PHPStan/Pint green; core action active |
| `client-new-thread:f78fb112-89cc-486c-a7c5-627009478b76` (`1747`, replacement for inactive `01a0881b-85ce-7f43-91d5-dd84fe064064`) | F9 invitation retention | `IssueAssessmentInvitation.php` and its focused test only; no migration/delete/scheduler | Audit and, only if the existing five-year/privacy/idempotency contract is proven, replace ad-hoc expiry with the accepted immutable retention policy | policy `8f2b5fe`, selection writer `f49ab2c` + `f1e038f`, and assessment writer `cce9ea0` accepted; replacement worktree created from coordinator baseline because the old task completed empty twice after restart |
