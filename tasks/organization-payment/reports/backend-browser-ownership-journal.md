...
- `WindowsRun` construction remains side-effect-free and may omit a publisher for inspection, but `claim()` requires a non-null callable coordinator-owned anchor publisher before creating claim/journal artifacts or allowing any managed spawn. Every durable generation publishes a fresh strict copy of `{generation, digest}` in order. An intent-generation publication failure leaves the local chain intact and fails before the managed `Popen`; callback mutation cannot alter supervisor state. Failures after `Popen` during owned-registration/intent-clear retain the process handle, advance the local chain, and leave the external anchor stale so recovery refuses it. No anchor is persisted beside the journal and no digest or secret is logged.
...
- Pure/mock unit suite: 56 tests passed after the final increment.
