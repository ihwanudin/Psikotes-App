...
| `/root/design_multi_case_schema` (`case-schema-sept10`) | F2 entitlement case-identity expansion | New `2026_09_10_000300` migration and its dedicated SQLite/PostgreSQL tests only | Add nullable generic entitlement case identity, exact DIRECT/LEGACY backfill, DASS exclusion, composite FK, partial uniqueness, immutable guard, and safe rollback refusal | Worker `127e578` passed its focused gates but independent review requested changes: SQLite rebuild must preserve existing grant children with FK disabled outside the transaction plus `foreign_key_check`; DIRECT preflight must prove exact two-way package composition; idempotent catalog checks must reject counterfeit indexes/FK/trigger function/ACL/RLS state. A corrective child commit is active; nothing is integrated. This remains slice 1/6 and must not contract old participant uniqueness or wire runtime |
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, implementation resumed by `/root/rereview_f9_manual_proof`) | F9 audit-retention repair | `GenericAssessmentResultPollProjection.php` plus its focused projection test only | Replace both hard-coded two-year audit expiries with the five-year Audit policy without changing poll semantics | Manual proof through generic result-store slices are integrated through `bcbfdfb`; callback-orchestrator worker `c6bff2d` was independently accepted and integrated as `640fa56` (main 19/153). Every slice has PHPStan 0 and green Pint/diff gates. Poll-projection retention is active from worker baseline `c6bff2d`; no migration, route, provider call, or live integration ownership |
