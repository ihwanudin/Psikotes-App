...
### Checkpoint private Windows ACL descriptor snapshot — accepted `49d6b87`

ABI kini berisi **29 signature**, termasuk `IsValidAcl`. Primitive private
mengambil dua descriptor snapshot dari live handle yang sama dan mewajibkan
self-relative/bounds/revision 1, pointer owner-group-DACL interior, serta DACL
present non-NULL, protected, tidak auto-inherited, dan revision 2. Seluruh rentang
`AclSize` harus contained sebelum traversal; digest memakai exact
`AclBytesInUse`. Hanya pointer allocation base di-`LocalFree` tepat sekali. Bukti root:
**24/24 tes**, `py_compile`, diff-check, dan final adversarial review **PASS**.

Bukti masih pure fake ABI. SID string/process-token owner match, ACE parsing
...
### Checkpoint process-token owner binding — accepted `9301369`

Primitive memakai urutan exact token-before → descriptor-1 → descriptor-2 →
token-after. Pseudo-handle `GetCurrentProcess` tidak ditutup; real
`OpenProcessToken(TOKEN_QUERY)` handle wajib non-inheritable dan ditutup exact
sekali. Probe `TokenUser` bounded harus mendapat error 122 lalu fill exact; SID
tidak boleh overlap header dan harus full-contained sebelum native validation.
Token SID wajib stabil, kedua owner exact match, dan `processTokenSid` immutable.
Bukti root accepted: **30/30 tes**, `py_compile`, diff-check, dan adversarial
review **PASS**.

Tes masih pure fake ABI. Groups/privileges/LocalSystem, role mask, impersonation,
effective `AccessCheck`, source-tree/cache/composition/attestor, native runtime,
...
### Checkpoint restricting SID corroboration — accepted `a5e3c98`

`IsTokenRestricted` hanya menguatkan authority `TokenRestrictedSids` class 11.
`FALSE` hanya sah setelah `SetLastError(0)` dan immediate `GetLastError()==0`;
nonzero berarti `true`, dan hasil wajib parity dengan snapshot class 11. Bukti
root accepted: **53/53 unittest**, `py_compile`, diff-check, dan adversarial
review **PASS** tanpa P1/P2.
...
### Checkpoint kontrak ordinary access provider — ADR-018

ADR-018 accepted untuk contract/pure-fake testing saja. Provider internal
memiliki independent externally provisioned ordinary token, seluruh raw handle,
derivation impersonation, target/descriptor live, native `AccessCheck`, dan
cache one-shot `attest/load/discard`. Coordinator/launcher tidak menerima
credentials, account/environment selector, raw handle, SID pointer, descriptor,
atau process handle.

Request/evidence baru pada anchor/execution untuk fresh/recovery mengikat
...
menolak
same user, LocalSystem, owner SID pada setiap group, fixed sensitive privilege,
AppContainer raw/nonzero, class-11/`IsTokenRestricted`, atau profile drift.
`TokenOrigin` observational dan distinct `AuthenticationId` hanya corroboration.
...
### Checkpoint candidate manifest closure ACL — accepted `3460588`

Candidate manifest closure kini menetapkan enam path ini sebagai exact required
sekaligus allowed:
...
**PASS** tanpa P1/P2. Ini hanya packaging integrity; tidak ada candidate build
...
### Checkpoint ACL source-binding dependency pinning — accepted `538c144`

Direct sibling dan stdlib dependency callables kini dipin berdasarkan module,
callable identity/type, serta Python metadata pre/post setiap panggilan dan
sebelum hasil diterima. Evidence root: **32/32 tests**, `py_compile`, dan
independent review **PASS** tanpa P1/P2.

CPython builtins, deeper stdlib internals, dan dependency mutation yang
sepenuhnya dipulihkan selama callback masih trusted/residual. Scope hanya
...
### Checkpoint broker-start identity dan structural evidence — accepted `b66ebba`, `bb369c6`, `9785eb5`

`b66ebba` menerima pure canonical supplied-data broker-start identity (**8/8
tests**) tanpa live composition proof. `bb369c6` menerima design boundary
ADR-020; `9785eb5` menerima bytes-only structural evidence codec (**12/12
tests**) dengan hasil explicit `structuralOnly`, bukan native policy,
provenance, atau admission.
...
`b0ae28e` menerima pure structural vendor-build artifact I8 yang mengikat
release/source artifact dan `composer.lock`, bounded package/file inventory,
runtime metadata, serta supplied build provenance. Evidence: focused
**11/11**, `py_compile`, `diff-check`, dan independent review **PASS** tanpa
P1/P2.

`8784215` menerima pure structural preparation-authorization artifact I9 yang
mengikat exact lima artifact statis pra-TLS, revocation-snapshot references,
dan destination/ACL intent dengan lifetime positif maksimum 10 menit. P2 path
sudah ditutup dengan batas komponen 255 byte, depth 64, dan penolakan alias
perangkat Win32. Evidence: focused **14/14**, `py_compile`, `diff-check`, dan
independent review **PASS**.

Scope kedua checkpoint hanya supplied canonical data. Tidak ada Composer/
filesystem/reproducibility authority, signature/trust, current freshness,
protected high-water, replay consumption, destination creation, ACL
application, admission, atau runtime authority. P15/P16/P17c/P18 tetap open,
