...
Tanggal observasi: 2026-09-07 16:34 ICT

Revisi sumber: `35a8ed9`
Lingkup: audit read-only atas kode/config, build sintetis lokal, dua tes SQLite
terisolasi, dan request pasif terhadap header publik. Tidak ada optimasi, migrasi,
load test, perubahan DNS/Cloudflare, penggunaan secret/data nyata, atau transaksi
provider.
...
Status di bawah menilai kesiapan *baseline yang dapat dibandingkan*, bukan menilai
...
|---|---|---|---|---|
| N+1 dan query budget | **PARTIAL** | Guard kode: portal SQLite `<=20`, preview PostgreSQL `<=16`, portal PostgreSQL list `<=3` dan detail `<=7`; tes consent lokal 20/20, 41 assertions | Guard hanya beberapa jalur; budget portal lokal tidak mencapai assertion | Pulihkan fixture, lalu catat query count 10/25/50 item pada SQLite dan PostgreSQL dengan revisi/data identik |
| PostgreSQL indexing dan EXPLAIN | **PARTIAL** | 38 migration; 55 deklarasi index non-unik eksplisit dan 79 deklarasi unique/index; 0 `EXPLAIN` nyata | Index tersedia tanpa bukti planner; write cost/selectivity belum terukur | Jalankan `EXPLAIN (ANALYZE, BUFFERS, WAL, FORMAT JSON)` di staging sintetis untuk shape prioritas |
| Cache dan invalidation | **MISSING** | Redis diwajibkan pada production/Compose; hanya 1 pemakaian facade cache eksplisit di app (`Cache::add`) | Tidak ada baseline hit/miss, key inventory, TTL/invalidation domain, atau stampede guard | Instrument hit/miss dan susun inventory key sebelum mengusulkan cache bank soal/response |
| Queue, jobs, backlog observability | **PARTIAL** | 2 worker Compose; 3 job class; polling scheduler 1/5 menit; limits 25/100 dan worker tries 5 | Tidak ada queue depth/oldest-age/SLA/alert implementation yang dapat diverifikasi | Seed backlog sintetis per queue dan ukur depth, oldest age, throughput, retry/failure serta recovery |
...
| Vite minification dan bundle size | **PARTIAL** | 2.317 modul; initial JS 535.460 B raw/166.785 B gzip; CSS entry 107.348/17.588 B; build 49,66 s | Chunk utama 520.430 B minified memicu warning >500 kB; tidak ada budget CI | Tambahkan pengukuran repeatable, bukan optimasi: manifest budget dan 3 build untuk variasi |
...
- `tests/Feature/Admin/OrganizationBillAccessTest.php` menetapkan budget render
  ...
  mencegah authorization query per row.
...
- Focused run `test_paginated_livewire_query_counts` gagal saat setup dengan
  `PACKAGE_COMPOSITION_INVALID`, sebelum assertion query count. Budget yang
  tertulis belum menjadi evidence hijau pada revisi ini.
- Tidak ditemukan enforcement global lazy-loading seperti
  `Model::preventLazyLoading`, dan tidak ada budget umum untuk seluruh route/API.
...
- **MISSING:** tidak ditemukan satu pun `EXPLAIN`/`EXPLAIN ANALYZE` nyata pada
  app, tests, database, atau config. Satu hasil pencarian kata "explains" pada
  nama tes bukan query-plan evidence.
...
| Initial JS graph (`app.tsx` + static imports) | 535.460 B raw / 166.785 B gzip |
| Main app chunk | 520.430 B raw / 160.929 B gzip |
| Entry CSS | 107.348 B raw / 17.588 B gzip |
| Seluruh CSS | 109.700 B raw / 17.968 B gzip |
| Total 3 WOFF2 | 51.490 B |
| Dynamic imports dari entry | 19 |

Build output menyatakan chunk warning dihitung *after minification*, sehingga
minification **READY** pada run ini. Initial JS masih di bawah budget contoh
200 kB gzip dari workflow performance, dan CSS di bawah 50 kB gzip. Namun status
area tetap PARTIAL karena main chunk melampaui warning raw 500 kB dan tidak ada
budget yang di-enforce di package scripts/CI.
...
1. Pulihkan tes query-budget yang saat ini gagal pada fixture.
2. Ambil query count/time dan EXPLAIN PostgreSQL pada dataset sintetis tetap.
3. Tambahkan telemetry read-only untuk cache dan queue backlog/age.
4. Reproduksi serta lokalisasi mixed-scheme header pada staging.
5. Ambil Lighthouse/waterfall dan tiga kali build untuk mengukur variasi.
