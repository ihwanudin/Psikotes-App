...
DB::transaction(function () use ($now): void {
    ...
    $participantA = upsertParticipant($branchA, $package, 'E2E Branch A Participant', '+6281234567890', 'E2E-A-001');
    $participantB = upsertParticipant($branchB, $package, 'E2E Branch B Participant', '+6281234567890', 'E2E-B-001');
    ...
    $commissionA = upsertCommission($branchA, $participantA, $ruleA, 'direct_order', 900001, 'E2E-A');
    $commissionB = upsertCommission($branchB, $participantB, $ruleB, 'direct_order', 900002, 'E2E-B');
    upsertWithdrawal($branchA, $branchAdmin, 'E2E-WD-A', 5000, $commissionA);
    upsertWithdrawal($branchB, $branchAdmin, 'E2E-WD-B', 7000, $commissionB);
...
function upsertCommission(int $branchId, int $participantId, int $ruleId, string $sourceType, int $sourceId, string $label): int
{
    ...
    return (int) DB::table('commission_entries')
        ->where('source_type', $sourceType)
        ->where('source_id', $sourceId)
        ->value('id');
}

function upsertWithdrawal(int $branchId, int $adminId, string $reference, int $amount, int $commissionEntryId): void
{
    ...
    $requestId = (int) DB::table('withdrawal_requests')->where('public_reference', $reference)->value('id');

    DB::table('withdrawal_request_items')->updateOrInsert(
        ['withdrawal_request_id' => $requestId, 'commission_entry_id' => $commissionEntryId],
        [
            'branch_id' => $branchId,
