...
/** Mirrors tests/Feature/AssessmentResults/ScoreSealedIstAnswerSetTest.php for RMIB. */
final class ScoreSealedRmibAnswerSetTest extends OrganizationPaymentTestCase
{
    use DatabaseTruncation;

    public function test_it_scores_one_exact_historical_rmib_source_into_a_stable_immutable_result(): void
    {
        $source = $this->rmibSource();
        ...
        [$first, $second] = app(RlsContextRunner::class)->runAsService(function () use (
            ...
            $scorer = app(ScoreSealedRmibAnswerSet::class);
        ...
        $this->assertInstanceOf(SealedRmibResult::class, $first);
        ...
        $this->assertSame('rmib-result:v1', $first->resultContractVersion);
        $this->assertSame($source->sourceChecksum, $first->sealedSourceChecksum);
        $this->assertSame([
            'id' => $scoringSourceId,
            'code' => 'rmib',
            'version' => 'F2-2026.09',
            'sourceFile' => 'rmib.json',
            'checksum' => hash('sha256', $this->canonicalPayload()),
        ], $first->scoringSource);
        $this->assertCount(12, $first->categories);
        $byCode = [];
        foreach ($first->categories as $category) {
            $byCode[$category['code']] = $category;
        }
        // Every response ranked position==rank within its group; expected
        // totals/ranks computed directly from database/seeders/data/rmib.json's
        // rotation table (independently verified, not copied from the scorer
        // under test).
        $this->assertSame(69, $byCode['Out']['rawScore']);
        $this->assertSame(11, $byCode['Out']['standardScore']);
        $this->assertSame('Outdoor', $byCode['Out']['category']);
        $this->assertSame(['lo' => 11, 'hi' => 11], $byCode['Out']['band']);
        $this->assertSame(72, $byCode['Med']['rawScore']);
        $this->assertSame(12, $byCode['Med']['standardScore']);
        $this->assertSame(1, $byCode['Med']['sourceScore']);
        $this->assertSame(1, $byCode['Med']['level']);
        $this->assertSame(45, $byCode['S.Se']['rawScore']);
        $this->assertSame(1, $byCode['S.Se']['standardScore']);
        $this->assertSame(10, $byCode['S.Se']['sourceScore']);
    ...
    public function test_it_requires_a_service_transaction_and_positive_internal_source_id_before_sql(): void
    {
        ...
        try {
            app(ScoreSealedRmibAnswerSet::class)->execute($this->rmibSource(), 1);
            $this->fail('Scoring must require an existing service transaction.');
        } catch (LogicException $exception) {
            $this->assertSame('SEALED_RMIB_RESULT_CONTEXT_REQUIRED', $exception->getMessage());
            $this->assertSame([], $queries);
        }

        app(RlsContextRunner::class)->runAsService(function () use (&$queries): void {
            try {
                app(ScoreSealedRmibAnswerSet::class)->execute($this->rmibSource(), 0);
    ...
    public function test_non_rmib_sealed_sources_are_rejected_before_the_scoring_source_query(): void
    {
        $queries = [];
        DB::listen(static function (QueryExecuted $query) use (&$queries): void {
            $queries[] = $query->sql;
        ...
        app(RlsContextRunner::class)->runAsService(function () use ($source): void {
            try {
                app(ScoreSealedRmibAnswerSet::class)->execute($source, 1);
                $this->fail('Only RMIB may enter the RMIB scorer.');
            } catch (UnexpectedValueException $exception) {
                $this->assertSame('SEALED_RMIB_RESULT_INSTRUMENT_UNSUPPORTED', $exception->getMessage());
            }
        });
    ...
    public function test_scoring_source_identity_and_payload_must_match_the_exact_rmib_row(): void
    {
        $source = $this->rmibSource();
    ...
    public function test_definition_shape_and_answer_values_fail_closed_without_answer_disclosure(): void
    {
        ...
        $wrongDefinition = $this->sealedSource(
            ...
            subtests: [['code' => 'G1', 'duration_seconds' => 60, 'item_count' => 1]],
            values: ['1'],
        );
        $privateAnswer = 'private-answer-value';
        $invalidAnswer = $this->rmibSource(valueOverride: $privateAnswer, valueOverrideItem: 1, asObject: true);

        foreach ([$wrongDefinition, $invalidAnswer] as $source) {
            try {
                $this->score($source, $scoringSourceId);
                $this->fail('Definition and answers must match the exact RMIB scorer contract.');
            } catch (UnexpectedValueException $exception) {
                $this->assertSame('SEALED_RMIB_RESULT_INVALID', $exception->getMessage());
    ...
    private function score(SealedGenericAnswerSet $source, int $scoringSourceId): SealedRmibResult
    {
        return app(RlsContextRunner::class)->runAsService(
            fn (): SealedRmibResult => app(ScoreSealedRmibAnswerSet::class)->execute($source, $scoringSourceId),
        );
    }
    ...
    private function rmibSource(
        ...
        $cells = $data['rotation'];
        usort($cells, static fn (array $a, array $b): int => [$a['group'], $a['position']] <=> [$b['group'], $b['position']]);
    ...
    private function canonicalData(): array
    {
        ...
        if (! is_array($data)) {
            throw new RuntimeException('Canonical RMIB scoring data is invalid.');
        }
    ...
    {
        $payload = file_get_contents(database_path('seeders/data/rmib.json'));
        if (! is_string($payload)) {
            throw new RuntimeException('Canonical RMIB scoring data could not be read.');
        }
