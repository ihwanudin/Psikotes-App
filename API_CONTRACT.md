...
- `GET  /sessions/:id/assets/:assetId/url` → `{url,expires_at}` (F2 IST reader Stage 1, 2026-09-21). Gerbang baca sama seperti `/sessions/:id/items`. `assetId` adalah ULID opaque yang muncul di payload `/items` suatu subtest bergambar (belum ada instrumen dengan pembaca yang menerbitkannya hari ini — FA/WU menyusul setelah #73). `url` adalah temporary URL disk privat (`Storage::temporaryUrl`, sama mekanisme dengan `identity`/`payment-proofs`), masa berlaku `min(10 menit, ends_at−now)` dan respons JSON membawa `Cache-Control: no-store, private`; URL itu sendiri, setelah diikuti, membawa header `Cache-Control` bawaan Laravel `ServeFile` (`no-store, no-cache, must-revalidate, max-age=0`). GET, bukan POST — menerbitkan URL tidak mengubah state tersimpan. `404 ASSET_NOT_FOUND` bila `assetId` tidak terdaftar untuk instrumen sesi itu (termasuk bila terdaftar untuk instrumen lain).
