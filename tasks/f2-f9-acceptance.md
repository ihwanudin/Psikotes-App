...
| F3 | partial | Pure eligibility/guardrail T-12..T-19. Versioned persistence (`eligibility_decision_versions`, append-only + G8 chain invariant) dan HTTP projection (`EligibilityDecisionController`, `ReviewInputController`) accepted on `deepseek/f3-f4-eligibility-narrative` (`d1ad798`..`3f67d6e`), PR #2 open against `codex/reconcile-org-f2-2026-09-15`, not yet merged. RLS/trigger/SECURITY DEFINER behavior proven against a real PostgreSQL 17.6 instance (`tests/Postgres/EligibilityDecisionRlsTest.php`), independently re-verified by coordinator on a separate container. | Integrasi ke review UI (F5) dan browser E2E. |
| F4 | partial | Composer bilingual deterministik T-20/T-21. Versioned persistence (`bilingual_narrative_versions`) dan HTTP projection (`BilingualNarrativeController`) accepted on the same branch/PR — system baseline only, not the psychologist-edited final text (see known gap below). PostgreSQL runtime evidence: `tests/Postgres/BilingualNarrativeRlsTest.php`, independently re-verified by coordinator. | Review UI dan keluaran dokumen. Psychologist-edited INTEGRATION text has no persistence home yet — open gap for whichever lane owns F5/F6 review output. |
