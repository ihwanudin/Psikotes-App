...
Assert-Contains $runner 'PRIMARY_FAILURE:' 'The original failure must remain visible when teardown also fails.'
Assert-Contains $runner 'CLEANUP_FAILURE:' 'Cleanup failure must be surfaced separately.'
Assert-Contains $runner 'UNKNOWN' 'Failed inventory must never be rendered as an apparent zero.'
Assert-Contains $runner '$containerLookupExit' 'Each exact container lookup must check native exit status.'
Assert-Contains $runner '$containerRemoveExit' 'Each exact container removal must check native exit status.'
Assert-Contains $runner '$networkInspectExit' 'Network inspection must check native exit status.'
Assert-Contains $runner '$networkRemoveExit' 'Network removal must check native exit status.'
Assert-Contains $runner '$containerInventoryExit' 'Final container inventory must check native exit status.'
Assert-Contains $runner '$networkInventoryExit' 'Final network inventory must check native exit status.'
Assert-Contains $runner "'NETWORK_LABEL_MISMATCH'" 'Network removal must require its exact task label.'
