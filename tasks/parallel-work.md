...
| `/root/design_multi_case_schema` (`case-schema-sept10`) | F2 entitlement case-identity expansion | Entitlement model plus DIRECT/LEGACY provisioning actions and their focused SQLite/PostgreSQL tests only | Dual-write server-derived case identity for every generic entitlement while DASS remains case-less | Schema worker `127e578` + correction `7cbc587` passed independent re-review and was integrated as `45f4a94` + `73660fb`; main SQLite 9/56, PHPStan 0, and Pint/diff gates are green, while worker disposable PostgreSQL passed 15/74. Dual-write slice 2/6 is active from worker baseline `7cbc587`; it must not enforce generic non-null, contract old uniqueness, modify grant/session schema, or wire readers |
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, implementation resumed by `/root/rereview_f9_manual_proof`) | F9 audit-retention repair | `ClaimAssessmentBillInvoice.php` plus its focused SQLite/PostgreSQL claim tests only | Move only claim audit retention to five years while leaving notification-message retention at two years | Manual proof through zero-price slices are integrated through `36710d1`; bill-finalization worker `4d26e70` was independently accepted and integrated as `d590a16` (main 11/82). Every slice has PHPStan 0 and green Pint/diff gates. Invoice-claim audit retention is active from worker baseline `4d26e70`; notification outbox TTL is explicitly outside scope, with no migration, route, provider call, or live integration ownership |
