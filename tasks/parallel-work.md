...
|---|---|---|---|---|
| F5 authoritative signing snapshot | `/root/review_ist` | New `app/Domain/Review/ReportSigningSnapshotComposer.php`, new unit test, and the existing signing transition policy/test only | Derive transition prerequisites from exact typed G7/override outcomes; fail closed on unrecorded audit or incomplete required recalculation | active from `b79f2f3`; review pending |
| F5 signing bypass architecture guard | `/root/review_p17c` | New `tests/Architecture/ReportSigningBypassGuardTest.php` only | Prevent application code outside the signing transition policy from requesting `SIGNED` through the generic state machine | active from `b79f2f3`; review pending |
| F4 T-20 bilingual determinism acceptance | `/root/review_session` | New `tests/Integration/Narrative/BilingualNarrativeDeterminismAcceptanceTest.php` only | Prove canonical same-input repeatability, order independence rejection, and deterministic G7 omission without changing production code | active from `b79f2f3`; review pending |
