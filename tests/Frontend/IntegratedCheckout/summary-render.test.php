...
function renderSummary(array $summary): array
{
    ...
    check($script->getAttribute('type') === 'application/json' && $script->getAttribute('id') === 'checkout-summary-v1', 'Inert script identity');
...
$cases['amount null zero and safe integer'] = function (): void {
    foreach ([[null, 'Belum tersedia'], [0, 'Rp 0'], [175000, 'Rp 175.000'], [9007199254740991, 'Rp 9.007.199.254.740.991']] as [$amount, $expected]) {
        $data = fixture();
        $data['payment'] = ['payer' => 'self', 'state' => 'unpaid', 'amountIdr' => $amount, 'amountSource' => $amount === null ? 'unavailable' : 'charge_snapshot', 'consultationRequested' => $amount === null ? null : false, 'actionAvailable' => false];
...
$cases['access consent and provenance'] = function (): void {
    foreach (['locked' => 'Prasyarat tes belum terpenuhi', 'partial' => 'Sebagian prasyarat tes belum terpenuhi', 'ready' => 'Prasyarat tes terpenuhi'] as $state => $label) {
        ...
        check(str_contains(text($xpath, '//section[@aria-labelledby="consent-heading"]'), 'Persetujuan untuk dokumen ini belum tercatat'), 'Required consent copy');
