...
| `/root/design_multi_case_schema` (`case-schema-sept10`) | F2 entitlement case-identity expansion | New `2026_09_10_000300` migration and its dedicated SQLite/PostgreSQL tests only | Add nullable generic entitlement case identity, exact DIRECT/LEGACY backfill, DASS exclusion, composite FK, partial uniqueness, immutable guard, and safe rollback refusal | Active from detached baseline `88cf854`. This is slice 1/6 after the accepted pure selection gate; it must not contract old participant uniqueness, modify grant/session migrations, or wire application readers/writers |
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, implementation resumed by `/root/rereview_f9_manual_proof`) | F9 audit-retention repair | `GenericAssessmentResultStore.php` plus its focused persistence test only | Replace its hard-coded two-year audit expiry with the five-year Audit policy without changing result persistence | Manual proof `6610392` + `855b780` was integrated as `8160eed` + `88cf854` (main 12/37); assessment-bill proof `ece6ce8` as `c0d3922` (22/99); organization-bill proof `7a05e84` as `5c264ce` (13/88); and handoff consume `81ccd4f` as `a971a0c` (8/82). Every slice was independently accepted and main PHPStan/Pint/diff gates are green. Generic result-store retention is active from worker baseline `81ccd4f`; no route, migration, or live integration ownership |
