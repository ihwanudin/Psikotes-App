...
## [Unreleased]
- **SCORING-4.5.0** (ADR-0032 PR3, 2026-09-23): **RMIB kini memakai aturan bertingkat untuk ranking tidak lengkap** (keputusan psikolog P3), bukan menolak seluruh sesi bila satu kelompok kurang lengkap. Per kelompok (12 posisi/rank): kurang satu peringkat → direkonstruksi (satu-satunya rank 1–12 yang belum dipakai), skor identik dengan kelompok lengkap; lebih dari satu peringkat hilang dan/atau ada duplikat rank → kelompok itu dikecualikan dari SEMUA kategori (bukan hanya kontribusinya sendiri), sesi lain tetap dinilai dan ditandai `review_required` untuk dibaca kualitatif oleh psikolog; dua kelompok atau lebih cacat → sesi `not_scorable`, administrasi ulang, tidak ada skor sebagian yang tersimpan. `RmibRawScoreCalculator::calculate()` memverifikasi setiap kelompok yang dipakai memuat tepat rank 1–12 berjumlah 78 sebelum menghitung kategori. `SealedRmibResult`'s `resultContractVersion` naik v2→v3 (field baru `reviewRequired`/`excludedGroups`) dan `engineVersion` naik v1→v2 (aturan skoring berubah, bukan sekadar orkestrasi). Tabel audit `assessment_scoring_attempts` (dari PR1) mencatat outcome `not_scorable` baru ini dengan `reason_code`, sudah otomatis terlihat di halaman review kegagalan penilaian tanpa perubahan kode (#139).
