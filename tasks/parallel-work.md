...
|---|---|---|---|---|
| `01a0881b-8590-7123-a9ce-640f7cbf75c3` (`2469`) | F2 PostgreSQL and snapshot migration | Sole migration owner plus PostgreSQL concurrency/RLS design; no allocator implementation | Audit the participant-request to trusted-service-transaction boundary read-only; freeze the smallest safe contract and focused synthetic proof without editing code | `000700` and snapshot EXPAND accepted; CONTRACT still deferred; concurrency design covers 7/7 scenarios. RLS boundary audit dispatched after the critical participant-context elevation mismatch was confirmed; concurrency test implementation remains held |
...
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, replacement for inactive `01a0881b-85ce-7f43-91d5-dd84fe064064`) | F9 invitation retention | `IssueAssessmentInvitation.php` and `tests/Feature/Admin/OrganizationInvitationTest.php` only; no migration/delete/scheduler | Preserve intentional reissue semantics while applying one immutable UTC audit anchor and the accepted five-year no-overflow retention policy; repair only the focused assessment-case fixture | Initial audit completed clean with no code: reissue is not an idempotent replay, privacy context is already PII-free, and the fixture predates mandatory integrated cases. Contract clarified and bounded implementation redispatched |
