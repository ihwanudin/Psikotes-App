...
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, implementation resumed by `/root/rereview_f9_manual_proof`) | F9 audit-retention repair | `FinalizeAssessmentBill.php` plus its focused SQLite/PostgreSQL tests only | Replace all three hard-coded two-year audit expiries with the five-year Audit policy without changing finalization semantics | Manual proof through result-outbox slices are integrated through `37a4f57`; zero-price worker `fcf2ac4` was independently accepted and integrated as `36710d1` (main 12/190). Every slice has PHPStan 0 and green Pint/diff gates. Assessment-bill finalization retention is active from worker baseline `fcf2ac4`; no migration, route, provider call, or live integration ownership |
