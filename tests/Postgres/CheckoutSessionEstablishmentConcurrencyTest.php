...
final class CheckoutSessionEstablishmentConcurrencyTest extends TestCase
{
    ...
    private function deletePackageItems(int $packageId): void
    {
        ...
        config()->set('database.connections.checkout_session_establishment_cleanup', [
            ...$config,
            'username' => 'org_test_owner',
        ]);

        try {
            DB::connection('checkout_session_establishment_cleanup')
                ->table('package_items')->where('package_id', $packageId)->delete();
        } finally {
            DB::purge('checkout_session_establishment_cleanup');
            config()->set('database.connections.checkout_session_establishment_cleanup', null);
        }
    }
