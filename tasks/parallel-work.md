...
|---|---|---|---|---|
| `01a0881b-8590-7123-a9ce-640f7cbf75c3` (`2469`) | F2 PostgreSQL and definition catalog | Sole migration owner plus catalog provider/schema/security/concurrency tests; no allocator/controller/route ownership | Repair the final SQLite U+0000 parity defect above `76a3e8f` + `c97be3a` | Real PostgreSQL concurrency proof is accepted. Re-review still rejected integration because SQLite text traversal stops at NUL and allowed U+0000 in nested identities despite domain rejection. Exact NUL rejection and adversarial parity tests are active; catalog, seeds, CONTRACT, allocator wiring, and active DB migration remain unintegrated/excluded |
| `01a0881b-86ec-7fc1-b0f3-99e1e5b99cb1` (`fa77`) | F2 pure multi-case selection boundary | New domain types/policy plus new unit tests only; no existing file, migration, catalog/provider, allocator/controller/route ownership | Encode only authority-safe replay/ambiguity decisions without choosing FIFO | Read-only audit completed: case foundation exists, but participant-scoped entitlement/session/grant/resolver constraints still prevent a second battery and durable retest grants do not exist. FIFO is authority-blocked, so the active pure slice must return typed ambiguity for multiple active or ready cases and remain database-free |
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, replacement for inactive `01a0881b-85ce-7f43-91d5-dd84fe064064`) | F9 audit-retention repair | `SetPaymentMethodActivation.php` plus its focused feature test only | Repair global payment-method activation audit after accepted identity-evidence slice | Identity worker `5ee4f9d` passed independent 11/36 plus PHPStan/Pint/diff and was accepted as coordinator `cafb51f`; main focused 11/36 green. Third bounded activation repair dispatched; remaining inventory stays unassigned |
