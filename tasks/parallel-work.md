...
|---|---|---|---|---|
| `01a0881b-8590-7123-a9ce-640f7cbf75c3` (`2469`) | F2 PostgreSQL and snapshot migration | Sole migration owner plus PostgreSQL concurrency/RLS/definition-catalog design; no allocator implementation | Design the smallest fail-closed versioned production SessionDefinition catalog/provider read-only without inventing missing instrument data | `000700` and snapshot EXPAND accepted; concurrency design covers 7/7 scenarios; ADR-0030 resolves the participant/service boundary; allocator core is independently green. Production definition authority remains the next blocker, so schema/API/test design is active before migration ownership is exercised |
| `01a0881b-86ec-7fc1-b0f3-99e1e5b99cb1` (`fa77`) | F2 multi-case/retest readiness | Read-only schema/API/domain audit after the accepted allocator; no code ownership during audit | Map participant-scoped constraints and freeze a history-safe second-battery/retest migration and selection sequence without caller-supplied case IDs | Allocator worker `f6a6f48` + `58a0a81` + `0a24f96` accepted as coordinator `f475a56` + `e07f2ad` + `55d0a59`; independent 152/471, PHPStan 0, Pint/diff clean. The explicitly excluded multi-case/retest gap is now under read-only audit while route wiring waits for definition authority |
