...
| `/root/wave1_s4_pg` (reused) | F2 R2 immutable IST result ledger; sole migration owner | `codex/f2-r2-ist-result-ledger`; `C:/Users/ThinkPad/.codex/worktrees/r2-wave1/Psikotes` | integration contains accepted schema stack `c2a3641` + `0a85d3a` + `0ae490b`; R2b worker baseline `4a38496` is patch-equivalent | migration is frozen; R2b owns only new `PersistSealedIstResult.php` and its new Feature test, including writer concurrency evidence there | Add the exact-replay, caller-transaction-owned writer over the accepted append-only schema. No reuse of Selection result history; no DASS, A1-D5, correction/retest, route, queue, report, or activation. | **R2a ACCEPTED schema-only; R2b ACTIVE.** QA independently passed full PG 15/122, focused adversaries 3/25, Pint, scoped PHPStan, cleanup 0/0, and confirmed no writer files in the accepted tree. PM accepted the re-verdict and reopened only the two quarantined writer files; migration and existing PostgreSQL test edits are forbidden |
