<?php

declare(strict_types=1);

namespace App\Actions\Registration;

use App\Actions\Notifications\EnqueueParticipantActivation;
use App\Models\AssessmentCase;
use App\Models\ConsentRecord;
use App\Models\Entitlement;
use App\Models\Order;
use App\Models\Participant;
use App\Models\PaymentMethod;
use App\Models\TestPackage;
use App\Registration\ConsentDocument;
use App\Security\RlsContext;
use App\Security\RlsContextRunner;
use App\Services\Referral\ReferralAttribution;
use App\Services\TestNumber\MonthlyTestNumberIssuer;
use Illuminate\Database\QueryException;
use Illuminate\Support\Str;
use Illuminate\Validation\ValidationException;

final readonly class RegisterParticipant
{
    public function __construct(
        private RlsContextRunner $runner,
        private ReferralAttribution $referrals,
        private MonthlyTestNumberIssuer $testNumbers,
        private EnqueueParticipantActivation $enqueueActivation,
    ) {}

    /**
     * @param  array<string, mixed>  $input
     */
    public function handle(
        array $input,
        int $packageId,
        ?string $paymentMethodCode,
        string $registrationToken,
        ?string $referralCookie,
    ): Participant {
        $payloadHash = $this->payloadHash([
            ...$input,
            'package_id' => $packageId,
            'payment_method_code' => $paymentMethodCode,
        ]);

        try {
            return $this->runner->run(
                new RlsContext('service'),
                fn (): Participant => $this->createOnce(
                    $input,
                    $packageId,
                    $paymentMethodCode,
                    $registrationToken,
                    $payloadHash,
                    $referralCookie,
                ),
            );
        } catch (QueryException $exception) {
            return $this->runner->run(
                new RlsContext('service'),
                function () use ($registrationToken, $payloadHash, $exception): Participant {
                    $participant = $this->lockedParticipant($registrationToken);
                    if ($participant === null) {
                        throw $exception;
                    }

                    $this->assertMatchingReplayGraph($participant, $payloadHash);

                    return $participant;
                },
            );
        }
    }

    /**
     * @param  array<string, mixed>  $input
     */
    private function createOnce(
        array $input,
        int $packageId,
        ?string $paymentMethodCode,
        string $registrationToken,
        string $payloadHash,
        ?string $referralCookie,
    ): Participant {
        $existing = $this->lockedParticipant($registrationToken);

        if ($existing !== null) {
            $this->assertMatchingReplayGraph($existing, $payloadHash);

            return $existing;
        }

        $package = TestPackage::query()
            ->where('is_active', true)
            ->whereNotNull('amount')
            ->where('amount', '>=', 0)
            ->where('currency', 'IDR')
            ->lockForUpdate()
            ->find($packageId);

        if ($package === null) {
            throw ValidationException::withMessages([
                'package_id' => 'Paket tidak tersedia untuk pendaftaran.',
            ]);
        }
        $packageTypes = $package->items()->lockForUpdate()->pluck('test_type')->all();
        $composition = $this->compositionKind($packageTypes);
        if ($composition === null) {
            throw ValidationException::withMessages([
                'package_id' => 'Paket tidak tersedia untuk pendaftaran.',
            ]);
        }

        $consultationSelected = (bool) ($input['include_consultation'] ?? false);
        $consultationAmount = $consultationSelected ? $package->consultation_amount : 0;

        if ($consultationSelected && $consultationAmount === null) {
            throw ValidationException::withMessages([
                'include_consultation' => 'Konsultasi psikolog belum tersedia untuk paket ini.',
            ]);
        }

        $totalAmount = (int) $package->amount + (int) $consultationAmount;
        $paymentMethod = $totalAmount > 0
            ? PaymentMethod::query()
                ->active()
                ->sharedLock()
                ->where('code', $paymentMethodCode)
                ->first()
            : null;

        if ($totalAmount > 0 && $paymentMethod === null) {
            throw ValidationException::withMessages([
                'payment_method_code' => 'Metode pembayaran tidak tersedia.',
            ]);
        }

        $assignment = $this->referrals->assignmentFromCookie($referralCookie);
        $participant = new Participant;
        $participant->forceFill([
            'registration_token' => $registrationToken,
            'registration_payload_hash' => $payloadHash,
            'package_id' => $package->id,
            'branch_id' => $assignment->branch->id,
            'referral_branch_id' => $assignment->branch->id,
            'referral_source' => $assignment->source,
            'source_system' => 'DIRECT_PUBLIC',
            'attribution_source' => $assignment->branch->ref_code,
            'full_name' => $input['full_name'],
            'gender' => $input['gender'],
            'birth_date' => $input['birth_date'],
            'education_level' => $input['education_level'],
            'intended_field' => $input['intended_field'],
            'phone' => preg_replace('/[^0-9+]/', '', (string) $input['phone']),
            'email' => $input['email'] ?? null,
            'test_number' => $this->testNumbers->issue(),
        ])->save();

        $now = now();
        $publicId = (string) Str::ulid();
        $assessmentCase = null;
        if ($composition === 'main') {
            $assessmentCase = new AssessmentCase;
            $assessmentCase->forceFill([
                'public_id' => $publicId,
                'participant_id' => $participant->id,
                'organization_id' => $participant->branch_id,
                'package_id' => $package->id,
                'origin' => 'DIRECT_PUBLIC',
                'intended_field_snapshot' => $participant->intended_field,
                'created_at' => $now,
                'updated_at' => $now,
            ])->save();
        }
        $isFree = $totalAmount === 0;
        $order = new Order;
        $order->forceFill([
            'public_id' => $publicId,
            'participant_id' => $participant->id,
            'assessment_case_id' => $assessmentCase?->id,
            'payment_method_id' => $paymentMethod?->id,
            'status' => $isFree ? 'paid' : 'pending',
            'amount' => $totalAmount,
            'currency' => $package->currency,
            'paid_at' => $isFree ? $now : null,
            'metadata' => [
                'pricing' => [
                    'package_code' => $package->code,
                    'package_amount' => (int) $package->amount,
                    'consultation_selected' => $consultationSelected,
                    'consultation_amount' => (int) $consultationAmount,
                ],
            ],
            'created_at' => $now,
            'updated_at' => $now,
        ])->save();
        $participant->entitlements()->createMany(
            collect($packageTypes)
                ->map(fn (string $testType): array => [
                    'order_id' => $order->id,
                    'test_type' => $testType,
                    'status' => $isFree ? 'ready' : 'locked',
                    'ready_at' => $isFree ? $now : null,
                    'created_at' => $now,
                    'updated_at' => $now,
                ])->all(),
        );

        $this->recordConsent($participant, ConsentDocument::for('psychotest'), true);
        $this->recordConsent($participant, ConsentDocument::for('dass'), true);

        if ($isFree) {
            $this->enqueueActivation->handle($order);
        }

        return $participant;
    }

    /** @param array<string, mixed> $input */
    private function payloadHash(array $input): string
    {
        ksort($input);

        return hash_hmac(
            'sha256',
            json_encode($input, JSON_THROW_ON_ERROR),
            (string) config('app.key'),
        );
    }

    private function assertMatchingPayload(Participant $participant, string $payloadHash): void
    {
        if (! is_string($participant->registration_payload_hash)
            || ! hash_equals($participant->registration_payload_hash, $payloadHash)) {
            throw ValidationException::withMessages([
                '_registration_token' => 'Token registrasi telah digunakan untuk data yang berbeda.',
            ]);
        }
    }

    private function lockedParticipant(string $registrationToken): ?Participant
    {
        $participants = Participant::query()
            ->where('registration_token', $registrationToken)
            ->orderBy('id')
            ->limit(2)
            ->lockForUpdate()
            ->get();

        if ($participants->count() > 1) {
            $this->rejectReplay();
        }

        return $participants->first();
    }

    private function assertMatchingReplayGraph(Participant $participant, string $payloadHash): void
    {
        $this->assertMatchingPayload($participant, $payloadHash);
        if ($participant->source_system !== 'DIRECT_PUBLIC' || ! is_int($participant->package_id)) {
            $this->rejectReplay();
        }

        $orders = Order::query()->where('participant_id', $participant->id)
            ->orderBy('id')->limit(2)->lockForUpdate()->get();
        if ($orders->count() !== 1) {
            $this->rejectReplay();
        }
        /** @var Order $order */
        $order = $orders->first();

        $package = TestPackage::query()->whereKey($participant->package_id)->lockForUpdate()->first();
        if ($package === null) {
            $this->rejectReplay();
        }
        $packageTypes = $package->items()->sharedLock()->pluck('test_type')->all();
        $composition = $this->compositionKind($packageTypes);
        if ($composition === null) {
            $this->rejectReplay();
        }

        $entitlements = Entitlement::query()->where('participant_id', $participant->id)
            ->orderBy('id')->lockForUpdate()->get();
        $entitlementTypes = $entitlements->pluck('test_type')->all();
        sort($entitlementTypes);
        sort($packageTypes);
        if ($packageTypes !== $entitlementTypes
            || $entitlements->contains(fn (Entitlement $entitlement): bool => $entitlement->order_id !== $order->id)) {
            $this->rejectReplay();
        }

        $cases = AssessmentCase::query()->where('participant_id', $participant->id)
            ->where('origin', 'DIRECT_PUBLIC')->orderBy('id')->limit(2)->lockForUpdate()->get();
        if ($composition === 'dass') {
            if ($order->assessment_case_id !== null || $cases->isNotEmpty()) {
                $this->rejectReplay();
            }

            return;
        }

        if (! is_int($order->assessment_case_id) || $cases->count() !== 1) {
            $this->rejectReplay();
        }
        /** @var AssessmentCase $case */
        $case = $cases->first();
        if ($case->id !== $order->assessment_case_id
            || $case->public_id !== $order->public_id
            || $case->participant_id !== $participant->id
            || $case->organization_id !== $participant->branch_id
            || $case->package_id !== $participant->package_id
            || $case->origin !== 'DIRECT_PUBLIC'
            || $case->intended_field_snapshot !== $participant->intended_field
            || ! $case->created_at->equalTo($order->created_at)) {
            $this->rejectReplay();
        }
    }

    /** @param array<array-key, mixed> $types */
    private function compositionKind(array $types): ?string
    {
        if (! array_is_list($types)) {
            return null;
        }
        foreach ($types as $type) {
            if (! is_string($type) || ! in_array($type, ['dass21', 'ist', 'kraepelin', 'papi', 'rmib'], true)) {
                return null;
            }
        }
        sort($types);
        if ($types === ['dass21']) {
            return 'dass';
        }

        return $types !== []
            && count($types) === count(array_unique($types))
            && in_array('dass21', $types, true)
            && count(array_diff($types, ['dass21'])) >= 1
            ? 'main'
            : null;
    }

    private function rejectReplay(): never
    {
        throw ValidationException::withMessages([
            '_registration_token' => 'Token registrasi tidak memiliki data pendaftaran yang konsisten.',
        ]);
    }

    private function recordConsent(Participant $participant, ConsentDocument $document, bool $accepted): void
    {
        ConsentRecord::query()->create([
            'participant_id' => $participant->id,
            'consent_type' => $document->type,
            'status' => $accepted ? 'accepted' : 'declined',
            'document_version' => $document->version,
            'document_hash' => $document->hash,
            'consented_at' => $accepted ? now() : null,
            'withdrawn_at' => null,
        ]);
    }
}
