...
point; the scoring JSON bytes for `papi.json` did not change between the old
baseline and `origin/main`.
...
| Duration                               | PRD 1.3 does not specify a PAPI duration. An older technical diagram writes `PAPI (20)`, but it does not establish a current approved unit or timer definition and cannot be converted to 1,200 seconds by assumption. New evidence in `tasks/handoffs/psychologist-duration-confirmation-2026-09-19.md` records the project owner's relayed WhatsApp confirmation from Rizqi Ulin Nuha, S.Psi., Psikolog: **PAPI 30 minutes** = **1,800 seconds**. | Bind 1,800 seconds into the final administration manifest and record the exact timeout, pause/resume, late-answer, and instruction/practice inclusion rules. Confirm whether the WhatsApp reply is sufficient formal evidence or requires a signed follow-up. | Total seconds are now evidenced, but timer behavior and formal manifest binding remain unresolved.                  | **PARTIAL**      |
...
4. What formal record binds the evidenced 1,800-second duration to the final
   PAPI manifest, and what are the approved timeout, pause/resume,
   late-answer, instruction, and practice-material rules?
