...
|---|---|---|---|
| Codex — F7 remainder | Dashboard/ledger/withdrawal ACCEPTED and merged (PR #5). **Remaining scope, authorized but NOT YET STARTED: proctoring persistence (`proctor_photos`/`proctor_logs`) + timeline UI**, tied to existing pure `ProctoringValidityPolicy` domain. Propose migration schema for Lead's review before creating it. **Dispatched 2026-09-20 as PROPOSAL ONLY** on `codex/f7-proctoring-proposal`: one document `tasks/handoffs/f7/proctoring-persistence-proposal.md` (schema + RLS per role, private photo storage/signed URL/retention, event-ingest endpoint mapped to T-25..T-28, timeline UI with real-HTTP test plan, link into `ProctoringValidityPolicy` — validity V1/V2/V3 stays the psychologist's). Detection, not prevention. No migration/code until Lead review. **Result: `codex/f7-proctoring-proposal` (`8976f05` + amendment `61d39f5`, off `650eea5`) → PR #18, proposal ACCEPTED, independently re-verified twice (Codex channel session, then Lead directly against `origin`) — ready to merge, pending user.** Exactly one file across both commits; still no migration, code, route, test or lockfile. Base citations spot-verified accurate (SPEC `proctor_photos`/`proctor_logs`, the `branch_fee_ledger` RLS pattern, `ProctoringValidityPolicy` ranges, `ProctoringEvent::allowedKinds`); detection-not-prevention explicit at line 260; carries the Filament route/real-HTTP-test lesson. **The amendment closed the two review gaps:** (1) mobile dead-stream persistence — per-session capture-cadence state (`expected_capture_min/max_seconds`, `last_photo_received_at`, `next_capture_due_at`, `close_reason` separating orderly close from silent death), server-side capture-gap inference, `terminal_session_state`, and three event kinds proposed for `ProctoringEventKind` (`CAMERA_REACTIVATION_ATTEMPT`, `CAPTURE_GAP_INFERRED`, `STREAM_DIED_WITHOUT_EVENT` — Lead confirmed all three are absent from the enum on `main`, and the document says so rather than claiming otherwise), satisfying `SPEC.md:205`'s `track.onended`/reactivation/marking requirement; (2) Spesifikasi v2.3 §9 now bound explicitly — camera inactive for any duration → V2, tab/fullscreen departure 1× → V2 (`SPEC.md:204-205`, `:211`, `:225-226`), with a no-threshold rule for schema/aggregation/UI and a note that `ProctoringValidityPolicy::requiresV2()` is already correct. Lead re-grepped the amended file for the psychologist-rejected figures (60 s / 3× / 8×): zero hits — the original gap was missing authority, not wrong numbers. Retention rewritten to the corrected **two-way** conflict (proctoring media: 90-day video vs 6-month photos; audit trail 5 y vs "log 2 tahun"), with psikotes 5 y / DASS 2 y confirmed as agreeing across SPEC, template v2.3 and PRIVACY_POLICY; no purge scheduling designed anywhere. Decision-list item 6 (photo cadence) recorded as already settled by `SPEC.md:203` (per-branch configurable, only the storage location open); item 3 (psychologist RLS predicate) deliberately left unlocked pending the DeepSeek F5 widening. **Migration turn: GLM's `report_documents` goes first; no proctoring migration until that lands and Lead re-opens this lane**, regardless of PR #18's accepted status. | New tables/migration (propose first), `app/Filament/**` additions, `app/Domain/Proctoring/**` (extend) | Other lanes' domains; `composer.json`/lockfiles unless coordinated |
...
| Codex — instrument authority pack (`codex/authority-pack-v2`, replacing `codex/instrument-authority-pack`) | Docs-only, no vendor/composer dependency. **Correction 2026-09-20 (found by Codex, verified by Lead):** `papi.md` and `ist.md` have been on `main` since `7dbea06` (2026-09-14), both BLOCKED 0/4; `kraepelin.md`/`rmib.md` exist only on the old branch (`2110519`), which is ~100 commits behind — earlier ledger text saying "PAPI dispatched / IST not started" was stale. **Dispatched 2026-09-20 as reconciliation, not new authoring:** fresh branch from `main`, cherry-pick `2110519`; update existing `papi.md` duration (30 min) and `rmib.md` (15 min) from `tasks/handoffs/psychologist-duration-confirmation-2026-09-19.md`, recomputing readiness honestly (other rows stay BLOCKED without new evidence); refresh `ist.md` facts against current `main` (ME learn/recall split and "Verbal (3)" stay BLOCKED — psychologist decision); bump all four packs' candidate baseline with a history note. Also investigate KRA-A7/RMIB-A7: exact hash method each audit used (raw bytes / LF vs CRLF / canonical JSON) — this is direct input to the `jsonb` checksum decision above; report only. **Result: `codex/authority-pack-v2` (2 commits, tip `8562c3c`, off `650eea5`) → PR #17, ACCEPTED by the Codex channel session, spot-checked by Lead, awaiting user merge.** Docs-only: 4 files under `tasks/handoffs/authority-pack/`, no code, seeder data or Lead-owned file. Reconciliation done as dispatched: `rmib.md`/`kraepelin.md` carried over from `2110519`, all four baselines bumped to `650eea5` with a history note naming the old `274c43d`. PAPI 1800 s / RMIB 900 s recorded as **PARTIAL**, not closed, carrying forward the open question whether a WhatsApp reply meets the packs' evidentiary bar. All four packs remain readiness **0/4**, import-ready NO, start-ready NO — no inflation. **KRA-A7/RMIB-A7 investigation, re-verified independently by the channel session** (it recomputed all five hash variants of `kraepelin.json` itself and matched Codex's figures): raw byte SHA-256 `68cd9bf…` — exactly what `InstrumentSeeder` computes — LF-normalized identical (file is already LF-only), CRLF `85141fd…`, minified `367eadf…`, key-sorted minified `0698cbb…`. The git blob is identical at the old audit baseline `a522f47` and at `650eea5` for both files, so **the old audit values (`ddc4cdf…`/`fc5d6db…`) are not reproducible by any documented method and must not be cited as verified evidence**. Conclusion — the raw-file byte SHA-256 is the defensible provenance fingerprint and must not be promoted to `template_checksum` — is consistent with the raw-text-column approach approved for the DeepSeek `jsonb` fix and is usable input for it now. Residual: KRA-A7/RMIB-A7 stay open for the audit owner (correct, not a gap). Queued follow-up, not yet authorized: cite v2.3's IST norm provenance (service-unit internal table from Master Kamus Tes IST) in `ist.md`. | New files only under `tasks/handoffs/authority-pack/**` | Any code, migration, or `database/seeders/data/**` edit; `composer.json`/lockfiles; this file, `tasks/parallel-work.md`, `tasks/f2-f9-acceptance.md` |
