...
- `GET  /sessions/:id/answers` → `{session_id,answers_revision,answers:[{item_no,value}]}` — memulihkan jawaban yang sudah diautosave saat peserta memuat ulang halaman mid-test. Gerbang baca: hanya `in_progress` dan sebelum `ends_at` (`SESSION_NOT_STARTED`/`SESSION_CLOSED`/`DEADLINE_EXCEEDED`); tidak pernah menulis, tidak pernah menyegel sesi yang lewat deadline. `answers_revision` dan baris `answers` dibaca dalam satu statement (bukan dua `SELECT` terpisah) agar tidak ada torn-read melawan autosave yang bersamaan. Tidak ada field selain `item_no`+`value`; tidak ada kunci jawaban, skor, atau status per-item.
- `GET  /sessions/:id/items` → `{session_id,instrument,version,subtests:[{code,items:[...]}],instructions}`. Gerbang baca sama seperti `/sessions/:id/answers`: hanya `in_progress` dan sebelum `ends_at` (`SESSION_NOT_STARTED`/`SESSION_CLOSED`/`DEADLINE_EXCEEDED`); tidak pernah menulis; `503 ASSESSMENT_ITEM_CONTENT_UNAVAILABLE` bila belum ada pembaca terdaftar untuk instrumen itu, atau bila pembaca yang terdaftar belum bisa menghasilkan konten. Bentuk `items` per instrumen ditentukan pembacanya sendiri, bukan endpoint ini. `instructions` (2026-09-21): teks administrasi per instrumen (mis. intro/contoh/penutup), `null` untuk instrumen/pembaca yang belum punya teks itu -- field ini selalu ada di bentuk respons, tidak pernah dihilangkan, supaya bentuknya stabil lintas instrumen. Server adalah satu-satunya sumber teks ini; klien tidak boleh menyimpan salinannya secara statis. **Kraepelin** (2026-09-21): 50 subtest `col_01..col_50`, masing-masing berisi tepat 28 `{position,value}` **sudah dalam urutan administrasi (bawah ke atas)** — `position=1` adalah angka paling bawah di lembar cetak, `position=28` paling atas. Sumber `kraepelin_grid.json` tersimpan dalam urutan lembar (baris 0 = atas); server membalik urutannya sekali di `KraepelinItemContentReader` sebelum respons dikirim. Klien TIDAK boleh membalik urutan apa pun lagi — satu pembalikan yang salah tempat memasangkan jawaban peserta dengan pasangan angka yang salah. **PAPI** (2026-09-21): satu subtest `ITEMS`, 90 `{item,statement_a,statement_b}` persis urutan sumber; `instructions` berisi `{intro,example:{statement_a,statement_b},answer_sheet_demo:{label,statement_a,statement_b},closing}`. **RMIB** (2026-09-21): satu subtest `POSITIONS`, 108 `{group,group_letter,position,job}` persis urutan sumber; `instructions` berisi `{text,write_preferred_jobs_prompt}`. `job` adalah SATU dari dua varian gender sumber (`job_male`/`job_female`) -- server yang memilih berdasar `participants.gender`, tidak pernah keduanya dikirim, tidak pernah nama field varian yang membocorkan track mana yang dipakai. Varian dipilih SEKALI saat sesi dimulai dan dikunci bersama sesi (`test_sessions.item_content_variant`, immutable setelah insert) -- bukan dibaca ulang dari profil peserta di setiap `/items`, supaya daftar pekerjaan tidak berubah di tengah tes kalau profil peserta diedit setelah mulai. Gender kosong/tidak dikenal saat sesi dimulai → start RMIB ditolak (`503 ASSESSMENT_ITEM_CONTENT_UNAVAILABLE`) sebelum baris sesi/grant tercipta, bukan menebak default. **IST** (`IstItemContentReader`, F2 IST reader Stage 1, 2026-09-21, ME ditambahkan 2026-09-22): subtest `SE/WA/AN` (multiple-choice) → `{code,answer_type:'multiple_choice',instructions,items:[{item,text?,options:{a..e}}]}` (`text` hanya ada bila soal punya kalimat pembuka — `WA` tidak); `GE` → `answer_type:'fill_in_word'`, `items:[{item,text}]`; `RA/ZR` → `answer_type:'fill_in_numeric'`, `items:[{item,text}]`. `ME` dua fase, ditentukan segmen berjalan sesi (`TimedSegmentSweep`, bukan field yang diminta klien): selama `ME_MEMORIZE`, `{code:'ME',answer_type:'multiple_choice',instructions,items:[],word_list:{BUNGA:[...5 kata],PERKAKAS:[...],BURUNG:[...],KESENIAN:[...],BINATANG:[...]}}`; selama `ME_ANSWER`, `items` berisi 20 `{item,options:{a..e}}` (tanpa `word_list`); selama segmen subtest lain, `ME` tidak menunjukkan salah satu (`items:[]`, `word_list` tidak ada). Kunci jawaban/`match_strategy` TIDAK PERNAH ada di respons ini — hanya di `instrument_versions` code=`ist` (norma skoring), tidak pernah dibaca pembaca item. `FA`/`WU` belum ada sama sekali di `main` (lihat PR #73) — seluruh instrumen IST tetap `503 ASSESSMENT_ITEM_CONTENT_UNAVAILABLE` (termasuk `start`, diperiksa sebelum attempt/grant dialokasikan) hanya jika `ist_items.json` kembali berstatus `draft` di tingkat atas (bukan kondisi hari ini — SE/WA/AN/GE/RA/ZR/ME semuanya `final`).
- `GET  /sessions/:id/assets/:assetId/url` → `{url,expires_at}` (F2 IST reader Stage 1, 2026-09-21). Gerbang baca sama seperti `/sessions/:id/items`. `assetId` adalah ULID opaque yang muncul di payload `/items` suatu subtest bergambar (belum ada instrumen dengan pembaca yang menerbitkannya hari ini — FA/WU menyusul setelah #73). `url` adalah temporary URL disk privat (`Storage::temporaryUrl`), masa berlaku `min(10 menit, ends_at−now)` dan respons JSON membawa `Cache-Control: no-store, private`. Berbeda dari `identity`/`payment-proofs` (yang masih memakai `Illuminate\Filesystem\ServeFile` bawaan Laravel): disk `ist-assets` memakai `ServeIstAssetController` sendiri (`serve=>false` di `config/filesystems.php`) karena `ServeFile` hanya mengirim `Cache-Control: no-store, no-cache, must-revalidate, max-age=0` pada respons sukses, bukan pada 403/404 — celah itu bisa membuat browser meng-cache 404 sesaat secara heuristik (RFC 9111) untuk URL yang, karena `expires`-nya presisi detik, bisa terbit identik dua kali dalam detik yang sama. `ServeIstAssetController` mengirim header itu di SETIAP cabang (sukses, 403, 404), dan tiap `temporaryUrl()` menambahkan `nonce` acak ke tanda tangan sehingga dua URL yang diterbitkan pada detik yang sama tidak pernah identik. GET, bukan POST — menerbitkan URL tidak mengubah state tersimpan. `404 ASSET_NOT_FOUND` bila `assetId` tidak terdaftar untuk instrumen sesi itu (termasuk bila terdaftar untuk instrumen lain).
- `POST /sessions/:id/events` — batch Kraepelin `{col,row,answer,client_ts_ms}[]` (insert-ignore per seq)
- `POST /sessions/:id/subtest/next` — F2 timed-segments stage 4 (2026-09-22). Pindah ke segmen berikutnya secara eksplisit (mis. peserta menekan "lanjut" sebelum jendela waktu segmen berakhir); server tetap satu-satunya sumber kebenaran jadwal (`TimedSegmentSweep`), permintaan ini hanya mempercepat transisi yang sudah akan terjadi otomatis saat deadline.
...
Kode error sesi stabil: `ENTITLEMENT_NOT_READY`, `RETEST_NOT_AUTHORIZED`, `SESSION_NOT_FOUND`, `ATTEMPT_ALREADY_EXISTS`, `SESSION_NOT_STARTED`, `SESSION_CLOSED`, `DEADLINE_EXCEEDED`, `AUTOSAVE_STALE_REVISION`, `AUTOSAVE_REVISION_GAP`, `MUTATION_PAYLOAD_MISMATCH`, `INVALID_ANSWER_BATCH`, `INVALID_SESSION_TRANSITION`, `ASSESSMENT_ITEM_CONTENT_UNAVAILABLE`, dan `ASSET_NOT_FOUND` (khusus `GET /sessions/:id/assets/:assetId/url`), dengan envelope `{error:{code,message,details}}`. Detail keberadaan resource lintas tenant tidak dibocorkan.
