...
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, continued by `/root/review_f9_lifecycle`) | F9 audit-retention repair | `IssueAssessmentBillInvoice.php` plus its focused test only | Replace only its hard-coded two-year audit expiry with the five-year Audit policy without changing invoice/provider behavior | Manual proof through invoice-claim slices are integrated through `f2add06`; integrated-checkout is integrated as `b5adb83`. Checkout lifecycle, handoff issuance, and session establishment are integrated as `079f9bc`, `91b048d`, and `5a15950` with main focused tests 23/261, 14/171, and 7/122. Invoice outcome worker `22f5cbf` passed independent review and is integrated as `e888155` with main 17/283; PHPStan/Pint/diff are green. `RecordAssessmentOutcome` remains unchanged because its two-year value is outbox/business retention. Invoice-issuance audit slice is active from worker baseline `22f5cbf`; `ProvisionAssessmentParticipant.php` remains exclusively owned by F2 and no migration, route, config, provider call, or live integration is owned here |
