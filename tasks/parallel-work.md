...
| `/root/wave1_s4_pg` (reused) | F2 R2 immutable IST result ledger; sole migration owner | `codex/f2-r2-ist-result-ledger`; `C:/Users/ThinkPad/.codex/worktrees/r2-wave1/Psikotes` | accepted integration head `8efbbcc`; writer baseline `c6ec235` | migration `2026_09_13_000100_create_generic_instrument_result_ledger.php`, new `PersistSealedIstResult.php`, its new Feature/PostgreSQL tests, and PostgreSQL XML only if required | Add an append-only FORCE-RLS case/session/source-bound IST ledger and exact-replay writer in schema-first then writer slices. No reuse of Selection result history; no DASS, A1-D5, correction/retest, route, queue, report, or activation. | R2a `ce636da` + exact-state repair `c6ec235` accepted and integrated as `c2a3641` + `0a85d3a`; independent fresh PG 12/97 with exact cleanup 0/0. R2b writer is active on `c6ec235`; migration is frozen and writer review remains required |
