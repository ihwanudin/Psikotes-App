...
| `01a0890c-53b1-7bf1-a6b3-59addda5b722` (`1747`, continued by `/root/review_f9_lifecycle`) | F9 audit-retention repair | `UpdateFundingPolicy.php` plus `FundingPolicyManagementTest.php` only | Replace only the funding-policy audit's hard-coded two-year expiry with five-year Audit policy | Manual proof through invoice-claim slices are integrated through `f2add06`; integrated-checkout is integrated as `b5adb83`. Checkout lifecycle, handoff issuance, and session establishment are integrated as `079f9bc`, `91b048d`, and `5a15950`; invoice outcome/permit and reservation audits are integrated as `e888155`, `ab3411f`, and `1ce2cce`. Assessment activation audit `1c0a8b5` is integrated as `5b90b0f`; main focused test passed 30/113 and reviewer downstream gate passed 59/1,918. All accepted slices have green Pint/diff and no new PHPStan findings. Funding-policy audit slice is active from worker baseline `1c0a8b5`; outbox two-year retention remains unchanged, F2 files remain exclusive, and no migration, route, config, panel test, provider call, or live integration is owned here |
