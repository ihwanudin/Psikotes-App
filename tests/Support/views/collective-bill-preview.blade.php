<div class="space-y-6">
    ...
    @php
        $previewFormId = 'collective-preview-'.$this->getId();
    @endphp
    <form id="{{ $previewFormId }}" wire:submit="review" class="space-y-4">
        <fieldset class="min-w-0 space-y-3" wire:loading.attr="aria-busy">
                ...
                <div wire:key="choice-{{ $id }}" class="space-y-2 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                    <label for="attempt-{{ $id }}" class="flex items-start gap-3">
                        <input id="attempt-{{ $id }}" type="checkbox" wire:click="toggleAttempt({{ $id }})"
                            @checked($selectedIndex !== null) aria-describedby="selection-help" class="fi-checkbox-input shrink-0" />
                    ...
                    @if ($selectedIndex !== null)
                        <label for="consultation-{{ $id }}" class="flex items-center gap-3">
                            <input id="consultation-{{ $id }}" type="checkbox"
                                wire:model.live="selection.{{ $selectedIndex }}.consultationRequested" class="fi-checkbox-input shrink-0" />
    ...
    {{-- Native form association keeps focus on the review button while Livewire
         temporarily disables inputs inside the submitted form. No focus script. --}}
    <x-filament::button type="submit" :form-id="$previewFormId" wire:loading.attr="aria-busy">Tinjau</x-filament::button>
